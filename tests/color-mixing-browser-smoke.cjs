...
const profile = fs.mkdtempSync(path.join(temporaryRoot, 'color-mixing-test-'));
const fixture = pathToFileURL(path.join(__dirname, 'fixtures/color-mixing-browser.html'));
try {
  for (const width of [1440, 390, 320]) {
    ...
    if (process.env.COLOR_MIXING_SCREENSHOTS) {
      fs.mkdirSync(process.env.COLOR_MIXING_SCREENSHOTS, { recursive: true });
      args.push(`--screenshot=${path.resolve(process.env.COLOR_MIXING_SCREENSHOTS, `lesson-${width}.png`)}`);
    }
  ...
  assert.ok(path.basename(resolved).startsWith('color-mixing-test-'));
