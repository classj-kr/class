...
for(const engine of ['chromium','webkit'])test(engine+': corrected phase grades and non-numeric food chains',{timeout:120000},async()=>{
    const root=path.resolve(__dirname,'../learning/inquiry/science-lab'),out=path.resolve(process.env.SCIENCE_TEST_ARTIFACTS || path.resolve(__dirname,'../docs/science-lab-audit-2026-09-20'), 'content-validity');fs.mkdirSync(out,{recursive:true});
