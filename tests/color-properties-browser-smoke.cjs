...
const profile = fs.mkdtempSync(path.join(temporaryRoot, 'color-properties-test-'));
const fixture = pathToFileURL(path.join(__dirname, 'fixtures/color-properties-browser.html'));
try {
  for (const width of [1440, 390, 320]) {
    ...
    if (process.env.COLOR_PROPERTIES_SCREENSHOTS) {
      fs.mkdirSync(process.env.COLOR_PROPERTIES_SCREENSHOTS, { recursive: true });
      args.push(`--screenshot=${path.resolve(process.env.COLOR_PROPERTIES_SCREENSHOTS, `lesson-${width}.png`)}`);
    }
  ...
  assert.ok(path.basename(resolved).startsWith('color-properties-test-'));
