...
const profile = fs.mkdtempSync(path.join(temporaryRoot, 'art-theory-test-'));
const fixture = pathToFileURL(path.join(__dirname, 'fixtures/art-theory-browser.html'));
try {
  for (const width of [1440, 390, 320]) {
    ...
    if (process.env.ART_THEORY_SCREENSHOTS) {
      fs.mkdirSync(process.env.ART_THEORY_SCREENSHOTS, { recursive: true });
      args.push(`--screenshot=${path.resolve(process.env.ART_THEORY_SCREENSHOTS, `lesson-${width}.png`)}`);
    }
    ...
    assert.equal(report.passed, 108);
  ...
  assert.ok(path.basename(resolved).startsWith('art-theory-test-'));
