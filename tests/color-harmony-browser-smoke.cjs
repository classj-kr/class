...
const profile = fs.mkdtempSync(path.join(temporaryRoot, 'color-harmony-test-'));
const fixture = pathToFileURL(path.join(__dirname, 'fixtures/color-harmony-browser.html'));
try {
  for (const width of [1440, 390, 320]) {
    ...
    if (process.env.COLOR_HARMONY_SCREENSHOTS) {
      fs.mkdirSync(process.env.COLOR_HARMONY_SCREENSHOTS, { recursive: true });
      args.push(`--screenshot=${path.resolve(process.env.COLOR_HARMONY_SCREENSHOTS, `lesson-${width}.png`)}`);
    }
  ...
  assert.ok(path.basename(resolved).startsWith('color-harmony-test-'));
