...
const profile = fs.mkdtempSync(path.join(temporaryRoot, 'visual-elements-test-'));
const fixture = pathToFileURL(path.join(__dirname, 'fixtures/visual-elements-browser.html'));
try {
  for (const width of [1440, 390, 320]) {
    ...
    if (process.env.VISUAL_ELEMENTS_SCREENSHOTS) {
      fs.mkdirSync(process.env.VISUAL_ELEMENTS_SCREENSHOTS, { recursive: true });
      args.push(`--screenshot=${path.resolve(process.env.VISUAL_ELEMENTS_SCREENSHOTS, `lesson-${width}.png`)}`);
    }
  ...
  assert.ok(path.basename(resolved).startsWith('visual-elements-test-'));
