...
(async()=>{
 ...
 try{for(const {game,file} of games){
  ...
  await page.evaluateOnNewDocument(()=>{if(!location.protocol.startsWith('http'))return;localStorage.setItem('classPlayerName','화면검증');let api;Object.defineProperty(window,'ClassroomMultiplayerLobby',{configurable:true,get:()=>api,set:value=>{api={...value,create:options=>{window.__uiLobby=value.create(options);return window.__uiLobby;}};}});});
  try{
