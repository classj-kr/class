...
(async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(()=>{const t=performance.now();drawMap(innerWidth,innerHeight);return{day:seasonDay(),type:terrainAtPixel(self.x,self.y).type,duration:performance.now()-t}});
