...
const output = path.resolve('.tmp/arithmetic-audit-20260923');
const baseUrl = 'http://localhost:6180';
(async () => {
  const { learningWorksheetCatalog: catalog } = await import('../learning/literacy-numeracy/arithmetics/lib/arithmetic-worksheets.ts');
  ...
  const getState = page => page.evaluate(() => {
    ...
    const tokens = stages.map(s => [...s.querySelectorAll('.a4-sheet')].map(sheet => [...sheet.children].filter(el => el.tagName !== 'HEADER').map(el => el.textContent + [...el.querySelectorAll('svg')].map(svg => svg.innerHTML).join('')).join('')).join('')).join('');
  ...
  async function worker() {
    while (cursor < catalog.length) {
      ...
      try {
        ...
        if (count && initial.rows && record.total !== initial.rows) record.issues.push('counter/row count differs');
        ...
        if (await panelButton.count()) {
          ...
          const items = page.locator('[role="dialog"] .trig-derivative-answer-item');
          if (await items.count()) {
            const answers = await items.evaluateAll(items=>items.map(item=>{
              const buttons=[...item.querySelectorAll('.trig-derivative-choice')];
            ...
            if (answers.every(a=>a.correct>=0)) {
              for(let i=0;i<answers.length;i++) await items.nth(i).locator('.trig-derivative-choice').nth(answers[i].correct).click();
              await page.locator('[role="dialog"] .trig-derivative-panel-grade').click();
        ...
        const reset = page.getByRole('button',{name:/^다시 (풀기|쓰기)$/});
  ...
  console.log(JSON.stringify({total:results.length,flagged:results.filter(r=>r.issues.length).length}));
