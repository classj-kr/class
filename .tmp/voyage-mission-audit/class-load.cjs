...
(async()=>{try{
 ...
 const keys=['discovery:batur-caldera','city:lisbon','discovery:milford-sound'];
 ...
 let completions=[];
 for(const [index,position] of [[0,{lat:-8.24,lon:115.38}],[1,{city:'리스본'}],[2,{lat:-44.62,lon:167.75,mode:'sea'}]]){
 ...
 const result={ok:true,students:33,placesPerStudent:3,answers:297,completed:33,uniqueRanks:true,maxLocalAckMs:maxAckMs};fs.writeFileSync('.tmp/voyage-mission-audit/class-result.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
