...
const output = path.resolve('.tmp/arithmetic-audit-verified-20260923');
...
(async () => {
  ...
  const selected = [53, 63, 111, 116];
  const previous = JSON.parse(await fs.readFile(path.join(output, 'results.json')));
  const all = catalog.filter((item,index) => selected.includes(index) || item.route.includes('box-measurement') || item.route.includes('grade-6-circle'));
  const results = previous.filter(item => !all.some(next => next.route === item.route)); let cursor = 0;
  const gradePanel = async page => { const button = page.locator('button.trig-derivative-panel-grade'); if(await button.count()) await button.click(); else { await page.getByRole('button',{name:/닫기$/}).click(); await page.getByRole('button',{name:'전체 채점',exact:true}).click(); await page.getByRole('button',{name:'답안 입력',exact:true}).click(); } }; 
  ...
  async function worker() {
    while (cursor < all.length) {
      const worksheet = all[cursor++], index = catalog.indexOf(worksheet);
