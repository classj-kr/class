...
const teacherRegistration = async (u) => [10,11].includes(Number(u?.id)) ? {school_id:1,school_name:"미리보기 초등학교",teacher_type:Number(u.id)===11?"관리자":"담임"} : null;
...
app.get("/preview/as/:id",(req,res)=>{res.cookie("preview_user",String(Number(req.params.id)),{httpOnly:true,sameSite:"strict"});res.redirect("/school-election/"+(["10","11"].includes(req.params.id)?"?mode=teacher":""));});
