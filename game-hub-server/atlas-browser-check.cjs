...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,...(process.env.CHROME_PATH ? {executablePath:process.env.CHROME_PATH} : {}),args:['--enable-webgl','--use-angle=swiftshader','--enable-unsafe-swiftshader']});try{
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/learning/inquiry/globe/`);await page.waitForFunction(()=>window.__atlasCheck?.isReady()&&__atlasCheck.map.getSource('atlas-spots'));await page.waitForTimeout(1800);
 await page.screenshot({path:path.join(screenshots,'atlas-desktop.png')});
 assert.equal(await page.locator('[data-lesson]').count(),29);
 ...
 const retryAnswer=await page.evaluate(async()=>{const {QUESTIONS}=await import('/learning/inquiry/globe/curriculum.mjs?v=20260920-22');return QUESTIONS.find(q=>q.id===document.getElementById('quizBody').dataset.questionId).answer;});
 await page.locator('[data-answer="'+retryAnswer+'"]').click();await page.locator('#nextQuestion').click();await page.locator('#returnMap').click();
 await page.locator('#topicSearch').fill('없는주제');assert.equal(await page.locator('[data-lesson]').count(),0);await page.locator('#topicSearch').fill('');assert.equal(await page.locator('[data-lesson]').count(),29);assert.equal(await page.locator('#levelFilter').count(),0);
 ...
 assert.equal(await page.locator("#flowMotion").count(),0);
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/learning/inquiry/world-geography/#density`);await page.waitForFunction(()=>window.__atlasCheck?.isReady()&&__atlasCheck.map.getSource('atlas-spots'));await page.waitForTimeout(1600);assert.equal(await page.locator('#lessonPractice').count(),0);
