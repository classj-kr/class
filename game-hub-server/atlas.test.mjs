...
test('64 fresh questions have unique stable ids, explanations and unambiguous option keys',()=>{
  assert.equal(QUESTIONS.length,64);assert.equal(new Set(QUESTIONS.map(q=>q.id)).size,64);
...
test('climate raster is complete, sourced and correctly oriented at representative locations',()=>{
  const grid=data('atlas-climate.json'),cells=decodeClimate(grid);assert.equal(cells.length,1440*720);assert.match(grid.source,/original 0.1 degree/);assert.equal(grid.sourceSha256.length,64);
  const at=(lng,lat)=>String.fromCharCode(cells[Math.floor((90-lat)/.25)*1440+Math.floor((lng+180)/.25)]);
...
test('old geography entry uses the shared app and the home menu has one atlas link',()=>{
  ...
  const home=read('index.html');assert.match(home,/<strong>지리 탐구<\/strong>/);assert.doesNotMatch(home,/href="learning\/inquiry\/world-geography\/"/);
