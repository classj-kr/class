...
const apps = [...catalog.matchAll(/<a class="level-entry available"[^>]*href="([^"/]+)\/"[^>]*><b>([^<]+)<\/b><span>([^<]+)<\/span><\/a>/g)].map(m=>{
  ...
  return {slug,grade:m[2],title:m[3],catalogLine:catalog.slice(0,m.index).split('\n').length,
