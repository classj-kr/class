...
### 조형 원리 작업실 — `design-principles/`

- 정해진 배치를 슬라이더로 바꾸던 화면을 자유 구성 작업 공간으로 교체했습니다. 예상 작성·단계별 실험·정답 판정은 없습니다.
- 원·사각형·삼각형을 추가하고 마우스·터치로 직접 옮깁니다. 도형 선택 후 방향키로 이동하며 Shift+방향키는 더 크게 움직입니다.
- 선택한 도형의 위치·크기·색 변경, 복제·삭제·앞뒤 순서 변경을 지원합니다. 모든 도형 편집과 전체 비우기는 되돌릴 수 있습니다. 한 번의 드래그나 슬라이더 조작은 되돌리기 한 번에 취소됩니다.
- 현재 구성을 기준으로 남기면 작업판 옆에서 바꾸기 전후를 비교할 수 있습니다. 모바일에서는 두 화면을 위아래로 보여줍니다. 비교는 선택 기능이며 자유 편집에 선행 절차가 없습니다.
- 여러 구성을 저장하고 다시 불러오거나 비교 기준으로 사용할 수 있습니다. 저장본은 이후 편집으로 바뀌지 않습니다.
- 작업·비교 기준·저장한 구성은 이 브라우저의 localStorage에 보관합니다. 저장이 불가능하면 화면에 알리며 SVG 파일로 내려받을 수 있습니다. 다른 기기와 동기화하지 않습니다.
