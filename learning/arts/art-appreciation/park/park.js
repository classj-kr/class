(function () {
  ...
  // 05번 전시실(입체·공간)과 100% 동일한 THREE.GLTFLoader 기반 3D 스캔 GLB 로딩 시스템
    ...
    },
    {
      ...
      quizBank: [
        ...
        { q: '피에타상은 누가 누구를 안고 있는 모습을 표현했을까요?', options: ['마리아가 죽은 예수를 안은 모습', '마리아가 아기 예수를 안은 모습', '제자가 쓰러진 예수를 부축하는 모습'], answer: 0, explain: '마리아가 죽은 예수를 무릎 위에 안고 있는 슬픈 순간을 표현했어요.' },
        { q: '머리에서 넓은 옷자락까지 이어지는 선을 따라가 보면, 피에타상은 어떤 구도로 안정감을 만들까요?', options: ['삼각형', 'S자 곡선', '가로로 긴 직사각형'], answer: 0, explain: '머리에서 넓게 퍼진 옷자락까지 이어지는 삼각형이 예수의 몸을 품으며 안정감을 만들어요.' }
      ]
    },
    {
    ...
    },
    {
      ...
      quizBank: [
        ...
        { q: '발밑에서 시선을 위로 옮겨 보면, 여신상이 높이 치켜든 오른손에 들고 있는 것은?', options: ['횃불', '글이 새겨진 판', '칼'], answer: 0, explain: '오른손에는 횃불을 높이 들고, 왼손에는 미국 독립 날짜가 새겨진 판을 안고 있어요.' }
      ]
    },
    {
  ...
  function makePark() {
    ...
    ZONES.forEach(zone => {
      loadZoneModel(zone);
      if (zone.id === 'sphinx') {
        // 높이 20.22m로 보정한 현재 GLB의 렌더링 바운드(약 24.1m x 44.5m)에
        // 플레이어 여유 0.42m만 더해, 보이지 않는 장벽 없이 몸통 바로 옆까지 접근하게 한다.
        const [width, length] = zone.collisionSize;
        zoneObstacles.push({ x: zone.position[0], z: zone.position[2], hx: width / 2 + .42, hz: length / 2 + .42 });
        return;
      }
      const baseW = Math.max(3.5, zone.realHeight * 0.4);
