(function () {
  ...
  // 한국 그림 보기에 서양 화가·제목이 섞이면 화풍만 보고도 답이 보인다. 보기는 같은 갈래에서 고른다.
  const KOREAN_WORK_IDS=new Set(['p01','p04','n03','n05','n06','n07','n08','s01','s02','s03','s06','s07','s08','s10','s11','d16']);
  const isKoreanWork=work=>KOREAN_WORK_IDS.has(work.id);
  ...
  function buildImageQuestion(room,mode,target) {
    ...
    const pick=works=>shuffledCopy(works.filter(work=>isKoreanWork(work)===isKoreanWork(target)).map(work=>work[config.key]).filter(value=>value&&value!==correct&&value!=='작자 미상'));
    // 제목은 같은 방 작품을 먼저 쓰고, 모자라면 다른 방의 같은 갈래 작품으로 채운다.
    const distractors=[...new Set([...(mode==='title'?pick(room.works):[]),...pick(allWorks)])].slice(0,3);
    const explanations={
