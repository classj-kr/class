const CHAPTERS = [
    {
        "num": 1,
        "title": "1장 · 우물가의 물레",
        "beats": [
            {
                "art": "01-well.webp",
                "emoji": "🧵",
                "left": [
                    "해도 뜨기 전에 우물가에서 물레 소리가 났습니다. 실을 잣는 아이가 있었지요. 그 집에는 새어머니와 딸 둘이 살았습니다.",
                    "하나는 새어머니가 낳은 딸이었지요. 다른 하나는 세상을 떠난 남편이 남기고 간 딸이었습니다. 새어머니는 두 아이를 아주 다르게 대했지요."
                ],
                "right": [
                    "제 딸은 하루 종일 놀아도 그만이었습니다. 남편이 남긴 딸은 새벽부터 밤까지 일을 해야 했지요. 물을 긷고 밥을 짓고 마당을 쓸었습니다.",
                    "그러고도 우물가에 앉아 실을 자아야 했습니다. 손끝이 갈라져도 쉴 수가 없었지요. 그래도 아이는 군소리 한 번 하지 않았습니다."
                ]
            },
            {
                "art": "01-well-2.webp",
                "emoji": "🧵",
                "left": [
                    "그날도 아이는 우물가에서 실을 잣고 있었습니다.",
                    "한참을 자으니 손끝이 갈라져 피가 났지요. 실을 감는 물레가락에도 피가 묻었습니다.",
                    "아이는 그것을 씻으려고 우물물에 담갔지요. 그런데 그만 손이 미끄러졌습니다."
                ],
                "right": [
                    "물레가락이 우물 속으로 쏙 빠져 버렸습니다.",
                    "아이는 우물 속을 들여다보며 발을 동동 굴렀지요. \"어떡해, 빠졌어!\"",
                    "그 물레가락은 집안에서 오래 쓰던 것이었습니다. 아이는 얼굴이 새하얗게 질려 새어머니에게 달려갔지요. 그러고는 사정을 말했습니다."
                ]
            }
        ]
    },
    {
        "num": 2,
        "title": "2장 · 우물 속으로",
        "beats": [
            {
                "art": "02-fall.webp",
                "emoji": "🕳️",
                "left": [
                    "새어머니는 눈을 부라렸습니다.",
                    "\"떨어뜨렸으면 네가 건져 와야지! 찾아오지 않으면 집에 들어올 생각도 하지 마라.\"",
                    "아이는 목이 꽉 막혔습니다. 다시 우물가로 터덜터덜 돌아왔지요.",
                    "해가 벌써 기울고 있었습니다."
                ],
                "right": [
                    "아이는 한참 우물 속을 들여다보았습니다. 깊고 컴컴해서 바닥이 보이지 않았지요. 아무리 봐도 물레가락은 보이지 않았습니다.",
                    "아이는 눈을 꼭 감았습니다. 그러고는 우물 속으로 뛰어들었지요. 눈앞이 캄캄해졌습니다.",
                    "귓가에서 바람 소리가 윙 하고 났지요."
                ]
            },
            {
                "art": "02-fall-2.webp",
                "emoji": "🕳️",
                "left": [
                    "몸이 아래로 아래로 떨어졌습니다. 한참을 그렇게 떨어졌지요.",
                    "그런데 이상하게 무섭지가 않았습니다. 물에 빠지는 느낌도 나지 않았지요.",
                    "아이는 조심조심 눈을 떠 보았습니다. 눈앞이 온통 환했지요. 아이는 풀밭에 누워 있었습니다."
                ],
                "right": [
                    "햇빛이 쏟아지는 넓은 들판이었습니다. 발밑에는 풀꽃이 가득 피어 있었지요. 어디에도 우물은 보이지 않았습니다.",
                    "하늘도 땅도 낯설었지요. 아이는 옷에 묻은 풀잎을 털고 일어났습니다. 그러고는 들판을 걷기 시작했지요.",
                    "어디로 가야 할지는 몰랐습니다. 나비가 앞장서듯 팔랑팔랑 날았지요."
                ]
            }
        ]
    },
    {
        "num": 3,
        "title": "3장 · 빵을 꺼내 주세요",
        "beats": [
            {
                "art": "03-bread.webp",
                "emoji": "🍞",
                "left": [
                    "한참을 걷는데 어디선가 목소리가 났습니다.",
                    "\"저를 꺼내 주세요! 다 타 버리겠어요!\"",
                    "아이는 걸음을 멈추고 두리번거렸지요. 길가에 빵 굽는 화덕이 하나 있었습니다. 문틈으로 연기가 새어 나오고 있었지요.",
                    "안을 들여다보니 빵이 가득했습니다. 빵들이 외치고 있었던 것이지요."
                ],
                "right": [
                    "가장자리가 벌써 거뭇해지고 있었습니다. 아이는 얼른 주걱을 찾아 들었지요. 그러고는 빵을 하나하나 꺼냈습니다.",
                    "화덕 안에서 뜨거운 김이 확 올라왔지요. 손이 발갛게 익었지만 아이는 멈추지 않았습니다. 마지막 하나까지 다 꺼냈지요."
                ]
            },
            {
                "art": "03-bread-2.webp",
                "emoji": "🍞",
                "left": [
                    "빵을 다 꺼내고 나자 빵들이 입을 모아 말했습니다.",
                    "\"고마워요. 하마터면 새까맣게 탈 뻔했어요.\"",
                    "아이는 손을 호호 불면서 빵을 가지런히 늘어놓았습니다. 식으라고 사이를 벌려 두었지요. 빵에서 김이 모락모락 났습니다."
                ],
                "right": [
                    "그러고는 손을 툭툭 털고 다시 길을 걸었습니다. 뒤에서 고소한 냄새가 따라왔지요. 남을 도우니 마음이 가벼웠습니다.",
                    "낯선 곳인데도 무섭지가 않았지요. 들판에는 해가 따뜻하게 내리쬐었습니다. 바람에 풀 냄새가 실려 왔지요."
                ]
            }
        ]
    },
    {
        "num": 4,
        "title": "4장 · 사과나무 아래에서",
        "beats": [
            {
                "art": "04-apples.webp",
                "emoji": "🍎",
                "left": [
                    "조금 더 가니 사과나무가 서 있었습니다. 가지가 땅에 닿을 만큼 늘어져 있었지요. 그때 머리 위에서 목소리가 났습니다.",
                    "\"저를 좀 흔들어 주세요. 열매가 너무 무거워서 가지가 부러지겠어요.\"",
                    "아이는 소리 나는 쪽을 올려다보았지요. 가지마다 빨간 사과가 주렁주렁 달려 있었습니다."
                ],
                "right": [
                    "사과나무가 부탁하고 있었던 것이지요. 아이는 나무 앞으로 다가갔습니다.",
                    "두 팔로 줄기를 안고 힘껏 흔들었지요. 사과가 우수수 떨어졌습니다. 머리에도 어깨에도 사과가 부딪혔지요.",
                    "아이는 나무가 가벼워질 때까지 흔들었습니다. 팔이 뻐근했지만 그만두지 않았지요. 머리에 나뭇잎이 잔뜩 앉았습니다."
                ]
            },
            {
                "art": "04-apples-2.webp",
                "emoji": "🍎",
                "left": [
                    "아이는 떨어진 사과를 하나하나 주웠습니다. 풀밭에 가지런히 쌓아 놓았지요. 벌레 먹은 것은 따로 골라 두었습니다.",
                    "나무가 홀가분한 듯 가지를 폈지요. 잎사귀가 사르르 흔들렸습니다.",
                    "\"이제야 살 것 같네요! 고마워요. 조심해서 가세요.\""
                ],
                "right": [
                    "아이는 사과 하나를 얻어 들고 다시 걸었습니다. 아삭아삭 씹으니 참 달았지요. 들판 저쪽에 작은 집이 보이기 시작했습니다.",
                    "굴뚝에서 연기가 오르고 있었지요. 아이는 그리로 걸음을 옮겼습니다."
                ]
            }
        ]
    },
    {
        "num": 5,
        "title": "5장 · 홀레 할머니의 집",
        "beats": [
            {
                "art": "05-holle.webp",
                "emoji": "🛏️",
                "left": [
                    "이윽고 아이는 작은 집 앞에 이르렀습니다. 문가에 할머니 한 사람이 서 있었지요. 아이보다 두 배는 커 보였습니다.",
                    "아이는 인사를 하려고 고개를 들었지요. 그런데 할머니의 이가 어찌나 큰지 저도 모르게 흠칫 물러섰습니다.",
                    "그러자 할머니가 껄껄 웃었지요. 웃는 얼굴은 하나도 무섭지 않았습니다."
                ],
                "right": [
                    "\"무서워하지 마라. 나는 홀레라고 한단다. 우리 집에서 일해 주지 않겠니?\"",
                    "\"이불을 잘 털어 주면 된단다. 깃털이 날려야 아래 세상에 눈이 오거든.\"",
                    "아이는 눈이 그렇게 오는 줄은 몰랐습니다."
                ]
            },
            {
                "art": "05-holle-2.webp",
                "emoji": "🛏️",
                "left": [
                    "아이는 고개를 끄덕였습니다. 집 안은 깨끗하고 아늑했지요. 홀레 할머니는 밥을 넉넉히 차려 주었습니다.",
                    "아이는 오랜만에 배불리 먹었지요. 할머니가 내준 따뜻한 이불에 눕자마자 스르르 잠이 들었습니다.",
                    "이튿날부터 아이는 이불을 털었지요."
                ],
                "right": [
                    "창가에서 이불을 힘껏 흔들었습니다. 깃털이 눈송이처럼 흩날렸지요. 아래 세상에는 그때마다 눈이 펑펑 내렸습니다.",
                    "아이는 그것이 참 신기했지요. 창밖으로 고개를 내밀고 한참을 내려다보았습니다. 저 아래 지붕들이 하얗게 덮여 갔지요."
                ]
            }
        ]
    },
    {
        "num": 6,
        "title": "6장 · 금빛 소나기",
        "beats": [
            {
                "art": "06-gold.webp",
                "emoji": "✨",
                "left": [
                    "아이는 날마다 부지런히 일했습니다. 이불을 털고 집을 쓸고 그릇을 닦았지요. 홀레 할머니는 아이를 아주 마음에 들어 했습니다.",
                    "한 번도 나무라는 일이 없었지요. 오히려 잘한다고 칭찬해 주었습니다.",
                    "\"네 손끝이 참 야무지구나.\""
                ],
                "right": [
                    "그런데 얼마 뒤 아이의 마음이 무거워졌습니다. 먹을 것도 넉넉하고 일도 할 만했지요. 그래도 자꾸 집 생각이 났습니다.",
                    "아이는 조심스레 입을 열었지요.",
                    "\"할머니, 저는 집이 그리워요.\"",
                    "할머니가 하던 일을 멈추었습니다."
                ]
            },
            {
                "art": "06-gold-2.webp",
                "emoji": "✨",
                "left": [
                    "홀레 할머니는 고개를 끄덕였습니다.",
                    "\"그럴 줄 알았다. 착하게 일했으니 상을 주마.\"",
                    "할머니는 아이를 큰 문 앞으로 데려갔지요.",
                    "\"이 문으로 나가면 집이란다.\" 문이 스르르 열렸습니다."
                ],
                "right": [
                    "아이가 문을 지나는 순간이었습니다. 위에서 금빛 소나기가 쏟아졌지요. 금이 머리끝부터 발끝까지 붙었습니다.",
                    "할머니가 물레가락을 건네주었지요. \"잃어버린 물레가락도 여기 있단다.\"",
                    "문을 다 빠져나오자 아이는 온몸이 금빛으로 반짝이는 채로 우물가에 서 있었습니다. 눈에 익은 마당이었지요."
                ]
            }
        ]
    },
    {
        "num": 7,
        "title": "7장 · 나도 갈래",
        "beats": [
            {
                "art": "07-lazy.webp",
                "emoji": "😏",
                "left": [
                    "금빛 아이가 집에 들어서자 온 집이 뒤집혔습니다. 새어머니의 눈이 번쩍했지요.",
                    "아이가 있었던 일을 이야기했습니다. 새어머니는 제 딸을 돌아보았지요.",
                    "\"너도 어서 우물에 들어가 보아라!\""
                ],
                "right": [
                    "게으른 딸은 우물가에 앉았습니다. 그런데 실을 잣기는 귀찮았지요. 물레는 손도 대지 않았습니다.",
                    "그 대신 일한 척하려고 가시로 손끝을 찔러 피를 냈지요. 그 피를 물레가락에 묻혀 우물에 던졌습니다.",
                    "그러고는 눈을 질끈 감고 저도 뛰어들었지요. 금 받을 생각에 마음이 급했습니다."
                ]
            },
            {
                "art": "07-lazy-2.webp",
                "emoji": "😏",
                "left": [
                    "딸도 눈을 뜨니 들판이었습니다. 딸은 길을 걷다 빵 화덕 앞을 지나게 되었지요.",
                    "\"저를 꺼내 주세요! 다 타 버리겠어요!\"",
                    "화덕에서 매캐한 냄새가 났습니다. 딸은 코를 막았지요. \"뜨거워서 싫어.\"",
                    "딸은 그냥 지나쳤고, 빵은 새까맣게 타 버렸습니다."
                ],
                "right": [
                    "조금 더 가니 사과나무가 딸을 불렀습니다.",
                    "\"저를 좀 흔들어 주세요!\"",
                    "딸은 코웃음을 쳤지요. \"팔 아프게 왜 내가 흔들어?\"",
                    "그러고는 콧노래를 부르며 걸어갔습니다. 사과나무는 힘없이 가지를 늘어뜨렸지요."
                ]
            }
        ]
    },
    {
        "num": 8,
        "title": "8장 · 문을 지나며",
        "beats": [
            {
                "art": "08-ending.webp",
                "emoji": "🖤",
                "left": [
                    "딸도 홀레 할머니 집에 이르렀습니다. 금을 받고 싶어서 일하겠다고 냉큼 대답했지요. 첫날은 그래도 열심인 척했습니다.",
                    "그런데 이튿날부터 늦잠을 잤지요. 이불도 몇 번 툭툭 치고 말았습니다. 깃털이 몇 개 날리다 말았지요.",
                    "딸은 하품을 했습니다. \"이만하면 됐지, 뭐.\""
                ],
                "right": [
                    "아래 세상에는 눈이 오다 말다 했습니다. 그릇은 쌓이고 방바닥에는 먼지가 앉았지요.",
                    "홀레 할머니는 아무 말도 하지 않았습니다. 그저 며칠을 조용히 지켜보았지요. 그러고는 어느 날 딸을 문 앞으로 데려갔습니다.",
                    "딸은 드디어 올 것이 왔다며 좋아했지요."
                ]
            },
            {
                "art": "08-ending-2.webp",
                "emoji": "🖤",
                "left": [
                    "딸은 신이 나서 문 앞에 섰습니다. 이제 금이 쏟아질 차례라고 생각했지요.",
                    "홀레 할머니가 조용히 말했습니다. \"네가 일한 만큼 받아 가거라.\"",
                    "딸이 두 손을 활짝 벌리고 냉큼 문을 지났지요."
                ],
                "right": [
                    "그런데 쏟아진 것은 금이 아니었습니다. 시커먼 송진이 주르륵 흘렀지요. 딸은 온몸이 새까매진 채 우물가에 섰습니다.",
                    "수탉이 그 꼴을 보고 크게 울었지요. \"꼬끼오, 더러운 아이가 왔네!\"",
                    "송진은 아무리 씻어도 지워지지 않았답니다."
                ]
            }
        ],
        "moral": "남이 받은 상만 보고 따라가면, 그 사람이 한 일까지 따라 하지는 못한답니다."
    }
];

function artFrame(src, emoji) {
    return `
        <div class="art-frame">
            <img src="images/${src}?v=20260918" alt="" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
            <div class="art-fallback" style="display:none">${emoji}</div>
        </div>`;
}

/* 표지 글도 말에 따라 갈아 끼우므로 상수로 뺐다. */
const COVER = {
    title: '홀레 할머니',
    intro: [
        '독일의 그림 형제가 모아 적은 옛이야기예요. 독일에서는 눈이 펑펑 내리면 홀레 할머니가 이불을 턴다고 말하곤 했답니다.',
        '우물 속으로 떨어졌더니 다른 세상이 나오는 짜임이에요. 그곳에서 만난 홀레 할머니는 어떤 분일까요?'
    ]
};

function coverPage() {
    const cv = CV();
    return `
        <div class="page page-cover">
            <div class="story-page-left story-page-left-full">
                ${artFrame('cover.webp', '❄️')}
                ${readBtnHtml()}
            </div>
            <div class="story-page-right">
                <h1 data-say="0">${cv.title}</h1>
                ${cv.intro.map((p, i) => `<p data-say="${i + 1}">${p}</p>`).join('')}
            </div>
        </div>`;
}

function tocPage() {
    const pageOf = num => {
        const idx = PAGES.findIndex(p => p.kind === 'spread' && p.chapter.num === num && p.isFirst);
        return idx >= 0 ? FOLIOS[idx].start : '';
    };
    const itemHtml = s => `
        <li>
            <button type="button" data-goto="${s.num}">
                <span class="toc-num">${s.num}</span>
                <span>
                    <strong>${s.title.replace(/^(\d+장|Chapter \d+) · /, '')}</strong>
                    <small>${T().page(pageOf(s.num))}</small>
                </span>
            </button>
        </li>`;
    const quizIdx = PAGES.findIndex(p => p.kind === 'quiz');
    const quizItemHtml = `
        <li>
            <button type="button" data-goto-kind="quiz">
                <span class="toc-num">❓</span>
                <span>
                    <strong>${T().quiz}</strong>
                    <small>${quizIdx >= 0 ? T().page(FOLIOS[quizIdx].start) : ''}</small>
                </span>
            </button>
        </li>`;
    const afterIdx = PAGES.findIndex(p => p.kind === 'after' && p.isFirst);
    const afterItemHtml = `
        <li>
            <button type="button" data-goto-kind="after">
                <span class="toc-num">📖</span>
                <span>
                    <strong>${T().after}</strong>
                    <small>${afterIdx >= 0 ? T().page(FOLIOS[afterIdx].start) : ''}</small>
                </span>
            </button>
        </li>`;
    const chs = CH();
    const half = Math.ceil(chs.length / 2);
    const leftItems = chs.slice(0, half).map(itemHtml).join('');
    const rightItems = chs.slice(half).map(itemHtml).join('') + quizItemHtml + afterItemHtml;
    return `
        <div class="page page-toc">
            <div class="story-page-left">
                <h2>${T().toc}</h2>
                <ul class="toc-list">${leftItems}</ul>
            </div>
            <div class="story-page-right">
                <h2 class="toc-h2-ghost" aria-hidden="true">${T().toc}</h2>
                <ul class="toc-list">${rightItems}</ul>
            </div>
        </div>`;
}

function spreadPage(chapter, beat, isFirst, isLast) {
    const badgeHtml = isFirst ? `<div class="spread-chapter-badge">${chapter.title}</div>` : '';
    const n = beat.left.length;
    const leftHtml = beat.left.map((p, i) => `<p data-say="${i}">${p}</p>`).join('');
    let rightHtml = beat.right.map((p, i) => `<p data-say="${n + i}">${p}</p>`).join('');
    // 교훈은 오른쪽 칸이 아니라 두 칸 아래 제 자리에 놓는다. 칸 안에 밀어 넣으면 넘친다.
    const moralHtml = isLast && chapter.moral ? `<p class="fable-moral">${chapter.moral}</p>` : '';
    return `
        <div class="page page-story${moralHtml ? ' spread-final' : ''}">
            <div class="spread-art">
                ${badgeHtml}
                ${artFrame(beat.art, beat.emoji)}
                ${readBtnHtml()}
            </div>
            <div class="spread-text">
                <div class="spread-text-left">${leftHtml}</div>
                <div class="spread-text-right">${rightHtml}</div>
                ${moralHtml}
            </div>
        </div>`;
}

/* 읽고 나서 — 소설 트랙과 같은 형식이다. 다만 동화틀은 쪽을 재서 나누지 않으므로
   왼쪽·오른쪽 글을 손으로 나누어 둔다. 끝 쪽은 그대로 두므로 학습 허브로 가는
   단추 자리를 여기서 뺄 필요가 없다. */
const AFTERWORD = {
    title: '읽고 나서',
    emoji: '🧹',
    spreads: [
        {
            art: 'end.webp',
            left: [
                "홀레 할머니가 이불을 털면 아래 세상에 눈이 내린다고 합니다. 독일에서는 눈이 오면 그렇게 말했지요.",
                "부지런한 아이는 우물에 빠진 물레가락을 주우러 뛰어듭니다. 무서웠을 텐데도요. 잃어버린 것을 제 손으로 찾으려 한 것입니다.",
                "빵과 사과나무가 말을 겁니다. 꺼내 달라, 흔들어 달라고요. 아이는 가던 길을 멈추고 그것을 합니다.",
                "게으른 딸은 같은 길에서 같은 말을 듣습니다. 그런데 그냥 지나가지요. 금을 얻으러 가는 길이었으니까요."
            ],
            right: [
                "홀레 할머니는 두 아이에게 똑같이 이불 터는 일을 맡겼습니다. 달라진 것은 그 일을 어떻게 했는지였지요.",
                "게으른 딸은 금을 바랐는데 왜 얻지 못했을까요?"
            ]
        }
    ]
};

function afterPage(spread, isFirst) {
    const head = isFirst ? `<h2>${AF().title}</h2>` : '';
    // 그림은 오른쪽 위 모서리에 붙고, 글을 뺀 나머지 자리를 다 차지한다.
    const art = spread.art ? `<div class="after-art">${artFrame(spread.art, AF().emoji)}</div>` : '';
    const col = (ps, from) => ps.map((t, i) => `<p data-say="${from + i}">${t}</p>`).join('');
    return `
        <div class="page page-after">
            <div class="after-col after-col-left">
                ${head}
                ${col(spread.left, 0)}
            </div>
            <div class="after-col after-col-right${spread.art ? ' after-col-image' : ''}">
                ${art}
                ${readBtnHtml()}
                ${col(spread.right, spread.left.length)}
                <p class="after-home"><a class="home-btn" href="../../../../../">${T().home}</a></p>
            </div>
        </div>`;
}

const QUIZ = [
    {
        "q": "아이가 우물에 뛰어든 까닭은 무엇인가요?",
        "choices": [
            "물레가락을 빠뜨려서",
            "실을 다 못 자아서",
            "빵을 태워서"
        ],
        "answer": 0
    },
    {
        "q": "우물 속 들판에서 아이가 처음 만난 것은 무엇인가요?",
        "choices": [
            "사과나무",
            "홀레 할머니",
            "빵 가마"
        ],
        "answer": 2
    },
    {
        "q": "사과나무가 아이에게 부탁한 것은 무엇인가요?",
        "choices": [
            "사과를 주워 달라",
            "가지를 흔들어 달라",
            "잎을 털어 달라"
        ],
        "answer": 1
    },
    {
        "q": "홀레 할머니 집에서 아이가 한 일은 무엇인가요?",
        "choices": [
            "이불을 털었다",
            "실을 자았다",
            "사과를 주웠다"
        ],
        "answer": 0
    },
    {
        "q": "부지런한 아이가 문을 지날 때 어떻게 되었나요?",
        "choices": [
            "송진이 쏟아졌다",
            "눈이 내렸다",
            "금이 쏟아졌다"
        ],
        "answer": 2
    },
    {
        "q": "게으른 딸은 저를 꺼내 달라는 소리를 듣고 어떻게 했나요?",
        "choices": [
            "빵을 꺼내 먹어 버렸다",
            "코를 막고 지나쳤다",
            "가마 문을 닫아 버렸다"
        ],
        "answer": 1
    },
    {
        "q": "홀레 할머니가 게으른 딸에게 한 말은 무엇인가요?",
        "choices": [
            "일한 만큼 받아 가라",
            "다시 오지 마라",
            "금을 주겠다"
        ],
        "answer": 0
    },
    {
        "q": "이 책을 읽고 난 반응으로 알맞지 않은 것은 무엇인가요?",
        "wide": true,
        "choices": [
                "아이가 손끝이 갈라져 피가 나도 군소리 한 번 하지 않은 것을 보면, 새어머니 집에서 그렇게 지내는 데 익숙했던 거지.",
                "손이 발갛게 익도록 빵을 다 꺼낸 것을 보면, 제 갈 길이 급한 중에도 부탁을 지나치지 못했어.",
                "게으른 딸이 가시로 손끝을 찔러 피를 낸 것을 보면, 일한 흉내만 내면 될 줄 알았던 거지.",
                "새어머니가 아이의 이야기를 듣자마자 제 딸도 우물에 들어가라고 한 것을 보면, 딸도 그만큼 부지런해지길 바랐구나."
            ],
        "answer": 3
    }
];

/* 푼 자취는 책을 덮을 때까지 남긴다. 쪽을 옮겼다 돌아와도 표시가 그대로다.
   자취는 보기의 원래 번호로 적어 두므로, 다시 그릴 때 자리가 바뀌어도 따라간다. */
/* 한글 문제와 영어 문제는 서로 다른 것을 묻는다. 그래서 자취도 말별로 따로 적는다.
   한 자리에 같이 적어 두면, 말을 바꿨을 때 아이가 누른 적 없는 보기에 표시가 앉는다. */
const QUIZ_WRONG = {};   // 말:문제 번호 → 잘못 고른 보기의 원래 번호들
const QUIZ_DONE = {};    // 말:문제 번호 → 맞혔는가
const QUIZ_ORDER = {};   // 말:문제 번호 → 보기를 늘어놓을 차례
const QK = i => LANG + ':' + i;
const quizDone = () => Object.keys(QUIZ_DONE).filter(k => k.startsWith(LANG + ':')).length;

/* 보기 차례는 책을 여는 순간 한 번만 정한다. 답의 자리를 외워 버리면 문제가 아니게 되니 섞되,
   쪽을 옮길 때마다 다시 섞으면 방금 고른 자리가 눈앞에서 움직여 어지럽다.
   책을 나갔다 들어오면 처음부터 다시 정해진다.
   섞는 것은 그리는 차례뿐이고, 채점은 data-choice에 담긴 원래 번호로 한다. */
function shuffledOrder(n) {
    const a = [...Array(n).keys()];
    for (let i = a.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [a[i], a[j]] = [a[j], a[i]];
    }
    return a;
}

function quizPage() {
    const items = QZ().map((item, i) => {
        const wrong = QUIZ_WRONG[QK(i)] || [];
        const solved = !!QUIZ_DONE[QK(i)];
        const order = QUIZ_ORDER[QK(i)] || (QUIZ_ORDER[QK(i)] = shuffledOrder(item.choices.length));
        const btns = order.map(ci => {
            const mark = solved && ci === item.answer ? ' correct' : (wrong.includes(ci) ? ' incorrect' : '');
            return `<button type="button" class="quiz-choice${mark}" data-choice="${ci}">${item.choices[ci]}</button>`;
        }).join('');
        return `
        <div class="quiz-item${solved ? ' graded' : ''}" data-qindex="${i}">
            <p class="quiz-question">${i + 1}. ${item.q}</p>
            <div class="quiz-choices${item.wide ? ' quiz-choices-stack' : ''}">${btns}</div>
        </div>`;
    }).join('');
    return `
        <div class="page page-quiz">
            <h2>${T().quiz}</h2>
            <p class="quiz-intro-text" id="quizProgress">${T().done(quizDone(), QZ().length)}</p>
            <div class="quiz-list">${items}</div>
        </div>`;
}


/* 영어판 — 한국어 원고를 줄 단위로 옮기지 않고 영어로 다시 썼다. */
const EN = {
    lang: 'en',
    cover: {
        title: 'Mother Holle',
        intro: [
            "One of the old tales the Brothers Grimm collected in Germany. In Germany people used to say that when the snow came down thickly, Mother Holle was shaking out her quilt.",
            "Falling down a well and finding another world at the bottom is the shape of it. Who is Mother Holle, down there?"
        ]
    },
    chapters: [
        {
            num: 1,
            title: 'Chapter 1 · The Spinning Wheel by the Well',
            beats: [
                {
                    art: '01-well.webp',
                    emoji: '🧵',
                    left: [
                        "Before the sun was up, there was the sound of a spinning wheel at the well.",
                        "A girl was spinning thread there.",
                        "The girl's father had died, and she lived with her stepmother.",
                        "Her stepmother had a daughter of her own.",
                        "And the stepmother treated the two girls very differently."
                    ],
                    right: [
                        "Her own daughter could play all day and nobody minded.",
                        "But this girl worked from dawn until night, fetching the water, cooking the meals, sweeping the yard.",
                        "After all that, she had to sit at the well and spin.",
                        "Her fingertips cracked, but she never once complained."
                    ]
                },
                {
                    art: '01-well-2.webp',
                    emoji: '🧵',
                    left: [
                        "That day too she was spinning at the well.",
                        "After a long while her fingertips cracked and bled.",
                        "There was blood on the spindle.",
                        "She dipped it in the well water to wash it off.",
                        "But her hand slipped."
                    ],
                    right: [
                        "The spindle went straight down into the well!",
                        "She looked into the well, hopping from foot to foot. \"Oh no — it's gone in!\"",
                        "That spindle had been in the house a long time.",
                        "Her face had gone white. She ran to her stepmother and told her."
                    ]
                }
            ]
        },
        {
            num: 2,
            title: 'Chapter 2 · Down the Well',
            beats: [
                {
                    art: '02-fall.webp',
                    emoji: '🕳️',
                    left: [
                        "Her stepmother glared at her.",
                        "\"You dropped it, so you can fetch it up!\"",
                        "\"And do not think of coming into this house without it.\"",
                        "There was a lump in the girl's throat,",
                        "and she trailed back to the well.",
                        "The sun was already going down."
                    ],
                    right: [
                        "She looked down into the well for a long time.",
                        "It was so deep and dark that she could not see the bottom.",
                        "Look as she might, there was no sign of the spindle.",
                        "The girl shut her eyes tight and jumped into the well.",
                        "Everything went black, and the wind roared in her ears."
                    ]
                },
                {
                    art: '02-fall-2.webp',
                    emoji: '🕳️',
                    left: [
                        "Down and down she fell, for a long while.",
                        "And oddly enough she was not frightened.",
                        "It did not feel like going into water either.",
                        "The girl opened her eyes carefully.",
                        "Everything was bright. She was lying on the grass!"
                    ],
                    right: [
                        "It was a wide meadow, with the sun pouring down and flowers all over the grass.",
                        "There was no well anywhere. The sky and the ground were strange to her.",
                        "She brushed the grass off her clothes, got up, and began to walk.",
                        "A butterfly went fluttering ahead of her."
                    ]
                }
            ]
        },
        {
            num: 3,
            title: 'Chapter 3 · Take the Bread Out',
            beats: [
                {
                    art: '03-bread.webp',
                    emoji: '🍞',
                    left: [
                        "She had walked a good way when a voice came from somewhere.",
                        "\"Take me out! I shall burn to a cinder!\"",
                        "The girl stopped and looked about her.",
                        "Beside the road was an oven, with smoke coming out of its door.",
                        "She looked in. It was full of bread, and the bread was calling out!"
                    ],
                    right: [
                        "The edges were already going brown.",
                        "The girl found a paddle and took it up,",
                        "and lifted the loaves out one by one.",
                        "Hot steam came billowing out of the oven,",
                        "and her hands went red with it, and she did not stop.",
                        "She got every last one of them out."
                    ]
                },
                {
                    art: '03-bread-2.webp',
                    emoji: '🍞',
                    left: [
                        "When the bread was all out, the loaves spoke together.",
                        "\"Thank you. We were very nearly burnt black.\"",
                        "The girl blew on her hands and laid the loaves out neatly,",
                        "with space between them to cool.",
                        "The steam rose gently off the bread."
                    ],
                    right: [
                        "Then she dusted off her hands and walked on.",
                        "A good smell came after her down the road.",
                        "Helping somebody had made her feel cheerful.",
                        "Strange as the place was, she was not frightened.",
                        "The sun lay warm over the meadow, and the wind brought the smell of grass."
                    ]
                }
            ]
        },
        {
            num: 4,
            title: 'Chapter 4 · Under the Apple Tree',
            beats: [
                {
                    art: '04-apples.webp',
                    emoji: '🍎',
                    left: [
                        "A little further on stood an apple tree. Its branches bent down almost to the ground.",
                        "Then a voice came from over her head.",
                        "\"Please shake me. The fruit is so heavy my branches will break.\"",
                        "The girl looked up. Every branch was hung with red apples."
                    ],
                    right: [
                        "It was the tree asking! She went up to it.",
                        "She put both arms round the trunk and shook hard.",
                        "The apples came showering down, bouncing off her head and shoulders.",
                        "She shook until the tree was light. Her arms ached, but she did not stop.",
                        "There were leaves all over her hair."
                    ]
                },
                {
                    art: '04-apples-2.webp',
                    emoji: '🍎',
                    left: [
                        "Then the girl picked the fallen apples up one by one",
                        "and piled them neatly on the grass.",
                        "She put the ones with worms in to one side.",
                        "The tree stretched out its branches as though it felt lighter,",
                        "and the leaves stirred softly."
                    ],
                    right: [
                        "\"That is better!\" said the tree. \"Thank you. Go carefully on your way.\"",
                        "The tree gave her an apple, and she walked on. It was very sweet when she bit into it.",
                        "Away across the meadow a small house came into sight, with smoke coming up from the chimney.",
                        "And she went toward it."
                    ]
                }
            ]
        },
        {
            num: 5,
            title: "Chapter 5 · Mother Holle's House",
            beats: [
                {
                    art: '05-holle.webp',
                    emoji: '🛏️',
                    left: [
                        "At last the girl came to the little house.",
                        "An old woman was standing at the door. She looked twice the girl's size.",
                        "The girl looked up to greet her.",
                        "But the old woman's teeth were so big that the girl started back.",
                        "The old woman laughed."
                    ],
                    right: [
                        "Her laughing face was not frightening at all.",
                        "\"Don't be afraid. My name is Holle. Would you come and work for me?\"",
                        "\"You only have to shake out the quilts well. When the feathers fly, it snows in the world below.\"",
                        "The girl had never known that was how snow came."
                    ]
                },
                {
                    art: '05-holle-2.webp',
                    emoji: '🛏️',
                    left: [
                        "The girl nodded. The house was clean and snug inside.",
                        "Mother Holle set out plenty of food for her.",
                        "The girl ate her fill for the first time in a long while.",
                        "She lay down in the warm bed and fell asleep at once.",
                        "From the next day, she shook the quilts."
                    ],
                    right: [
                        "She shook them hard at the window, and the feathers went flying like snowflakes.",
                        "Every time, the snow came down thickly in the world below.",
                        "The girl thought that was wonderful.",
                        "She put her head out of the window and watched the roofs far below turn white."
                    ]
                }
            ]
        },
        {
            num: 6,
            title: 'Chapter 6 · A Shower of Gold',
            beats: [
                {
                    art: '06-gold.webp',
                    emoji: '✨',
                    left: [
                        "The girl worked hard every day —",
                        "shaking the quilts, sweeping the house, washing the dishes.",
                        "Mother Holle was very pleased with her.",
                        "She never once found fault,",
                        "and told her instead that she did it well.",
                        "\"You have a good pair of hands on you.\""
                    ],
                    right: [
                        "But after a while something began to weigh on the girl.",
                        "There was plenty to eat and the work was bearable.",
                        "Still, she kept thinking about home.",
                        "She spoke up carefully. \"Mother Holle, I miss my home.\"",
                        "Mother Holle stopped what she was doing."
                    ]
                },
                {
                    art: '06-gold-2.webp',
                    emoji: '✨',
                    left: [
                        "Mother Holle nodded.",
                        "\"I thought as much. You have worked well, so you shall have your reward.\"",
                        "She took the girl to a great door.",
                        "\"Go through this door and you are home.\"",
                        "The door swung slowly open."
                    ],
                    right: [
                        "The moment the girl passed through, a shower of gold came down from above.",
                        "The gold stuck to her from head to foot.",
                        "Mother Holle handed her the spindle. \"And here is the spindle you lost.\"",
                        "On the other side, the girl stood at the well, shining gold. It was her own yard."
                    ]
                }
            ]
        },
        {
            num: 7,
            title: 'Chapter 7 · I Want to Go Too',
            beats: [
                {
                    art: '07-lazy.webp',
                    emoji: '😒',
                    left: [
                        "The whole house was in an uproar at the sight of the golden girl,",
                        "and the stepmother's eyes lit up.",
                        "The girl told them everything that had happened,",
                        "and the stepmother turned to her own daughter.",
                        "\"Then you get into that well at once!\""
                    ],
                    right: [
                        "The lazy daughter sat down at the well.",
                        "Spinning was too much bother, so she never touched the wheel.",
                        "She pricked her finger on a thorn and smeared the blood on the spindle, to make it look like work.",
                        "Then she dropped it in the well and jumped in after it. She was in a hurry for the gold."
                    ]
                },
                {
                    art: '07-lazy-2.webp',
                    emoji: '😒',
                    left: [
                        "She too opened her eyes in the meadow. On the road she came past the bread oven.",
                        "\"Take me out! I shall burn to a cinder!\"",
                        "There was a burnt smell coming out of the oven.",
                        "She held her nose. \"Too hot. No thank you.\"",
                        "She walked past, and the bread burned black."
                    ],
                    right: [
                        "A little further on, the apple tree called to her.",
                        "\"Please shake me!\"",
                        "At first she did not even pretend to hear.",
                        "Then she sniffed. \"Why should I shake it and tire my arms?\"",
                        "She went off humming, and the apple tree let its branches down again."
                    ]
                }
            ]
        },
        {
            num: 8,
            title: 'Chapter 8 · Through the Door',
            beats: [
                {
                    art: '08-ending.webp',
                    emoji: '🖤',
                    left: [
                        "She came to Mother Holle's house too. Wanting the gold, she said yes to the work at once.",
                        "The first day she pretended to work hard.",
                        "But from the second day she slept late.",
                        "She gave the quilt a few taps, and a handful of feathers came out.",
                        "She yawned. \"That will do.\""
                    ],
                    right: [
                        "Down below, the snow came and stopped and came again.",
                        "The dishes piled up and dust settled on the floor.",
                        "Mother Holle did not say a word. She simply watched for a few days.",
                        "Then one day she took the daughter to the door.",
                        "The daughter was delighted. Here it came at last!"
                    ]
                },
                {
                    art: '08-ending-2.webp',
                    emoji: '🖤',
                    left: [
                        "She stood at the door in high spirits. Now the gold would come, she thought.",
                        "Mother Holle said quietly, \"You shall have exactly what you have worked for.\"",
                        "The daughter held out both hands and hurried through."
                    ],
                    right: [
                        "But it was not gold. Black pitch poured over her head.",
                        "She stood at the well, black from head to foot.",
                        "The cockerel saw the state of her and crowed, \"Our dirty girl is home!\"",
                        "No amount of washing ever got the pitch off."
                    ]
                }
            ]
        }
    ],
    quiz: [
        {
            q: 'Why did the girl jump into the well?',
            choices: ['Her stepmother told her to fetch the spindle back', 'She wanted to see the other world', 'She was running away from home'],
            answer: 0
        },
        {
            q: 'What did the girl come to first in the meadow below?',
            choices: ['The apple tree', 'Mother Holle’s house', 'The oven full of bread'],
            answer: 2
        },
        {
            q: 'What did the apple tree ask for?',
            choices: ['To be watered', 'To be shaken so the branches would not break', 'To be left alone'],
            answer: 1
        },
        {
            q: 'What happens in the world below when Mother Holle’s quilts are shaken?',
            choices: ['It snows', 'It rains', 'The wind gets up'],
            answer: 0
        },
        {
            q: 'Why did the first girl want to go home?',
            choices: ['She did not like the work', 'Mother Holle sent her away', 'She was homesick all the same'],
            answer: 2
        },
        {
            q: 'What did the lazy daughter do at the oven and the apple tree?',
            choices: ['She helped the oven but not the tree', 'She walked straight past both of them', 'She took the bread and the apples'],
            answer: 1
        },
        {
            q: 'What came down on the lazy daughter at the door?',
            choices: ['Black pitch', 'Gold', 'Snow'],
            answer: 0
        },
        {
            q: 'Which reaction to this book does NOT fit?',
            wide: true,
            choices: [
                "Her fingers cracked and bled and still she never complained — that was how life was at her stepmother’s house.",
                "She pulled out every loaf until her hands were scarlet — even in a hurry she could not walk past someone asking.",
                "The other girl pricked her finger on a thorn for the blood — she thought a show of work would be enough.",
                "The stepmother sent her own daughter down the well the moment she heard the story — she wanted her to become just as hard-working."
            ],
            answer: 3
        }
    ],
    afterword: {
        title: 'After Reading',
        emoji: '🧹',
        spreads: [
            {
                art: 'end.webp',
                left: [
                    "When Mother Holle shakes out her quilt, they say, it snows in the world below. That is what people in Germany used to say about snow.",
                    "The hard-working girl jumps in after the spindle that fell down the well. She must have been frightened. She wanted to get back what she had lost with her own hands.",
                    "The bread and the apple tree speak to her. Take me out, shake me down. And she stops what she is doing and does it.",
                    "The lazy daughter hears the same words on the same road. But she walks straight past, because she is on her way to get gold."
                ],
                right: [
                    "Mother Holle gave both girls the same work: shaking the quilts. What was different was how each of them did it.",
                    "The lazy daughter wanted the gold. Why did she not get it?"
                ]
            }
        ]
    },
    words: {
        'cover.webp': [
            { word: 'thickly', meaning: '펑펑', sentence: 'When the snow came down thickly.' },
            { word: 'shake out', meaning: '털다', sentence: 'Mother Holle was shaking out her quilt.' },
            { word: 'quilt', meaning: '이불', sentence: 'Shaking out her quilt.' },
            { word: 'at the bottom', meaning: '바닥에', sentence: 'Finding another world at the bottom.' }
        ],
        '01-well.webp': [
            { word: 'spinning wheel', meaning: '물레', sentence: 'The sound of a spinning wheel at the well.' },
            { word: 'spin', meaning: '실을 잣다', sentence: 'A girl was spinning thread there.' },
            { word: 'fetch', meaning: '길어 오다', sentence: 'Fetching the water, cooking the meals.' },
            { word: 'sweep', meaning: '쓸다', sentence: 'Sweeping the yard.' },
            { word: 'complain', meaning: '군소리하다', sentence: 'She never once complained.' }
        ],
        '01-well-2.webp': [
            { word: 'spindle', meaning: '물레가락', sentence: 'There was blood on the spindle.' },
            { word: 'dip', meaning: '담그다', sentence: 'She dipped it in the well water.' },
            { word: 'slip', meaning: '미끄러지다', sentence: 'And her hand slipped.' },
            { word: 'go white', meaning: '새하얗게 질리다', sentence: 'Her face had gone white.' }
        ],
        '02-fall.webp': [
            { word: 'glare at', meaning: '눈을 부라리다', sentence: 'Her stepmother glared at her.' },
            { word: 'a lump in one’s throat', meaning: '목이 메임', sentence: "There was a lump in the girl's throat." },
            { word: 'trail back', meaning: '터덜터덜 돌아가다', sentence: 'She trailed back to the well.' },
            { word: 'look as she might', meaning: '아무리 봐도', sentence: 'Look as she might, there was no sign of it.' },
            { word: 'roar', meaning: '윙 하고 울다', sentence: 'The wind roared in her ears.' }
        ],
        '02-fall-2.webp': [
            { word: 'oddly enough', meaning: '이상하게도', sentence: 'And oddly enough she was not frightened.' },
            { word: 'meadow', meaning: '들판', sentence: 'It was a wide meadow.' },
            { word: 'brush off', meaning: '털다', sentence: 'She brushed the grass off her clothes.' },
            { word: 'flutter', meaning: '팔랑팔랑 날다', sentence: 'A butterfly went fluttering ahead of her.' }
        ],
        '03-bread.webp': [
            { word: 'cinder', meaning: '숯덩이', sentence: 'I shall burn to a cinder!' },
            { word: 'oven', meaning: '가마', sentence: 'Beside the road was an oven.' },
            { word: 'loaf', meaning: '빵 덩이', sentence: 'She lifted the loaves out one by one.' },
            { word: 'paddle', meaning: '주걱', sentence: 'The girl found a paddle.' },
            { word: 'billow', meaning: '확 올라오다', sentence: 'Hot steam came billowing out.' }
        ],
        '03-bread-2.webp': [
            { word: 'burnt black', meaning: '새까맣게 탄', sentence: 'We were very nearly burnt black.' },
            { word: 'blow on', meaning: '호호 불다', sentence: 'The girl blew on her hands.' },
            { word: 'dust off', meaning: '툭툭 털다', sentence: 'She dusted off her hands.' },
            { word: 'cheerful', meaning: '기분 좋은', sentence: 'Helping somebody had made her feel cheerful.' }
        ],
        '04-apples.webp': [
            { word: 'bend down', meaning: '늘어지다', sentence: 'Its branches bent down almost to the ground.' },
            { word: 'shake', meaning: '흔들다', sentence: 'Please shake me.' },
            { word: 'trunk', meaning: '줄기', sentence: 'She put both arms round the trunk.' },
            { word: 'shower down', meaning: '우수수 떨어지다', sentence: 'The apples came showering down.' },
            { word: 'bounce off', meaning: '부딪혀 튀다', sentence: 'Bouncing off her head and shoulders.' }
        ],
        '04-apples-2.webp': [
            { word: 'pile', meaning: '쌓다', sentence: 'And piled them neatly on the grass.' },
            { word: 'worm', meaning: '벌레', sentence: 'She put the ones with worms in to one side.' },
            { word: 'stretch out', meaning: '펴다', sentence: 'The tree stretched out its branches.' },
            { word: 'come into sight', meaning: '보이기 시작하다', sentence: 'A small house came into sight.' },
            { word: 'chimney', meaning: '굴뚝', sentence: 'With smoke coming up from the chimney.' }
        ],
        '05-holle.webp': [
            { word: 'greet', meaning: '인사하다', sentence: 'The girl looked up to greet her.' },
            { word: 'twice one’s size', meaning: '두 배는 큰', sentence: 'She looked twice the girl’s size.' },
            { word: 'start back', meaning: '흠칫 물러서다', sentence: 'The girl started back.' },
            { word: 'quilt', meaning: '이불', sentence: 'You only have to shake out the quilts.' },
            { word: 'feather', meaning: '깃털', sentence: 'When the feathers fly, it snows below.' }
        ],
        '05-holle-2.webp': [
            { word: 'snug', meaning: '아늑한', sentence: 'The house was clean and snug inside.' },
            { word: 'eat one’s fill', meaning: '배불리 먹다', sentence: 'The girl ate her fill.' },
            { word: 'snowflake', meaning: '눈송이', sentence: 'The feathers went flying like snowflakes.' },
            { word: 'thickly', meaning: '펑펑', sentence: 'The snow came down thickly in the world below.' }
        ],
        '06-gold.webp': [
            { word: 'find fault', meaning: '나무라다', sentence: 'She never once found fault.' },
            { word: 'weigh on', meaning: '마음을 무겁게 하다', sentence: 'Something began to weigh on the girl.' },
            { word: 'bearable', meaning: '할 만한', sentence: 'The work was bearable.' },
            { word: 'speak up', meaning: '입을 열다', sentence: 'She spoke up carefully.' }
        ],
        '06-gold-2.webp': [
            { word: 'I thought as much', meaning: '그럴 줄 알았다', sentence: 'I thought as much.' },
            { word: 'reward', meaning: '상', sentence: 'You shall have your reward.' },
            { word: 'shower', meaning: '소나기', sentence: 'A shower of gold came down.' },
            { word: 'stick to', meaning: '붙다', sentence: 'The gold stuck to her from head to foot.' }
        ],
        '07-lazy.webp': [
            { word: 'uproar', meaning: '뒤집힘, 소동', sentence: 'The whole house was in an uproar.' },
            { word: 'light up', meaning: '번쩍하다', sentence: "The stepmother's eyes lit up." },
            { word: 'bother', meaning: '귀찮은 일', sentence: 'Spinning was too much bother.' },
            { word: 'prick', meaning: '찌르다', sentence: 'She pricked her finger on a thorn.' },
            { word: 'smear', meaning: '묻히다', sentence: 'Smeared the blood on the spindle.' }
        ],
        '07-lazy-2.webp': [
            { word: 'hold one’s nose', meaning: '코를 막다', sentence: 'She held her nose.' },
            { word: 'burnt smell', meaning: '매캐한 냄새', sentence: 'There was a burnt smell coming out.' },
            { word: 'pretend to hear', meaning: '들은 척하다', sentence: 'She did not even pretend to hear.' },
            { word: 'tire', meaning: '아프게 하다, 지치게 하다', sentence: 'Why should I shake it and tire my arms?' },
            { word: 'let down', meaning: '늘어뜨리다', sentence: 'The apple tree let its branches down again.' }
        ],
        '08-ending.webp': [
            { word: 'pretend', meaning: '~인 척하다', sentence: 'She pretended to work hard.' },
            { word: 'sleep late', meaning: '늦잠 자다', sentence: 'From the second day she slept late.' },
            { word: 'tap', meaning: '툭툭 치다', sentence: 'She gave the quilt a few taps.' },
            { word: 'pile up', meaning: '쌓이다', sentence: 'The dishes piled up.' },
            { word: 'yawn', meaning: '하품하다', sentence: 'She yawned.' }
        ],
        '08-ending-2.webp': [
            { word: 'in high spirits', meaning: '신이 나서', sentence: 'She stood at the door in high spirits.' },
            { word: 'work for', meaning: '일해서 얻다', sentence: 'Exactly what you have worked for.' },
            { word: 'pitch', meaning: '송진', sentence: 'Black pitch poured over her head.' },
            { word: 'no amount of', meaning: '아무리 ~해도', sentence: 'No amount of washing ever got the pitch off.' },
            { word: 'the state of', meaning: '~의 꼴', sentence: 'The cockerel saw the state of her.' }
        ],
        'end.webp': [
            { word: 'used to say', meaning: '~라고 말하곤 했다', sentence: 'That is what people used to say about snow.' },
            { word: 'with one’s own hands', meaning: '제 손으로', sentence: 'To get back with her own hands what she had lost.' },
            { word: 'stop what one is doing', meaning: '하던 일을 멈추다', sentence: 'She stops what she is doing and does it.' },
            { word: 'walk straight past', meaning: '그냥 지나가다', sentence: 'And she walks straight past.' },
            { word: 'on the way', meaning: '가는 길에', sentence: 'What each of them had done on the way there.' }
        ]
    }
};

/* 글은 두 벌이다. 위쪽 단추를 누르면 EN 쪽으로 갈아 끼우고 쪽을 다시 짠다.
   영어 원고가 없는 책은 단추가 아예 뜨지 않는다. */
/* ── 우리말 낱말 ────────────────────────────────────────────
   쪽마다 아이가 막히는 우리말을 골라 둔다. 영어 낱말과는 고르는 잣대가 다르다.
   영어는 숙어가 걸림돌이고, 우리말은 옛말이나 잘 안 쓰는 말이 걸림돌이다. */
const WORDS_KO = {
    "cover.webp": [
        { w: "옛이야기", k: "옛날부터 사람들 입으로 전해 온 이야기.", s: "독일의 그림 형제가 모아 적은 옛이야기예요." },
        { w: "우물", k: "땅을 파서 물을 얻는 곳.", s: "우물 속으로 떨어졌더니 다른 세상이 나오는 짜임이에요." },
        { w: "분", k: "사람을 높여 부르는 말.", s: "그곳에서 만난 홀레 할머니는 어떤 분일까요?" }
    ],
    "01-well.webp": [
        { w: "세상을 떠나다", k: "죽어서 이 세상에 없게 되다.", s: "다른 하나는 세상을 떠난 남편이 남기고 간 딸이었습니다." },
        { w: "대하다", k: "남을 어떤 마음으로 맞다.", s: "새어머니는 두 아이를 아주 다르게 대했지요." },
        { w: "새벽", k: "날이 밝기 시작하는 이른 때.", s: "남편이 남긴 딸은 새벽부터 밤까지 일을 해야 했지요." },
        { w: "갈라지다", k: "살이 터져 벌어지다.", s: "손끝이 갈라져도 쉴 수가 없었지요." }
    ],
    "01-well-2.webp": [
        { w: "발을 동동 구르다", k: "안타까워서 발을 자꾸 구르다.", s: "아이는 우물 속을 들여다보며 발을 동동 굴렀지요." },
        { w: "집안", k: "한 가족이 이루는 집.", s: "그 물레가락은 집안에서 오래 쓰던 것이었습니다." },
        { w: "사정", k: "일이 그렇게 된 까닭.", s: "그러고는 사정을 말했습니다." }
    ],
    "02-fall.webp": [
        { w: "건지다", k: "물에 빠진 것을 꺼내 올리다.", s: "\"떨어뜨렸으면 네가 건져 와야지! 찾아오지 않으면 집에 들어올 생각도 하지 마라.\"" },
        { w: "해가 기울다", k: "해가 서쪽으로 넘어가다.", s: "해가 벌써 기울고 있었습니다." },
        { w: "컴컴하다", k: "빛이 없어 아주 어둡다.", s: "깊고 컴컴해서 바닥이 보이지 않았지요." },
        { w: "귓가", k: "귀의 바로 옆.", s: "귓가에서 바람 소리가 윙 하고 났지요." }
    ],
    "02-fall-2.webp": [
        { w: "풀꽃", k: "풀에서 피는 작은 꽃.", s: "발밑에는 풀꽃이 가득 피어 있었지요." },
        { w: "낯설다", k: "처음 보아 눈에 익지 않다.", s: "하늘도 땅도 낯설었지요." },
        { w: "앞장서다", k: "여럿 가운데 맨 앞에서 가다.", s: "나비가 앞장서듯 팔랑팔랑 날았지요." }
    ],
    "03-bread.webp": [
        { w: "두리번거리다", k: "무엇을 찾느라 고개를 이리저리 돌리다.", s: "아이는 걸음을 멈추고 두리번거렸지요." },
        { w: "새어 나오다", k: "틈으로 조금씩 빠져나오다.", s: "문틈으로 연기가 새어 나오고 있었지요." },
        { w: "가장자리", k: "물건의 바깥쪽 끝 부분.", s: "가장자리가 벌써 거뭇해지고 있었습니다." },
        { w: "익다", k: "뜨거운 것에 데어 빛깔이 변하다.", s: "손이 발갛게 익었지만 아이는 멈추지 않았습니다." },
        { w: "화덕", k: "불을 피워 빵이나 음식을 굽는 아궁이.", s: "길가에 빵 굽는 화덕이 하나 있었습니다." }
    ],
    "03-bread-2.webp": [
        { w: "입을 모으다", k: "여럿이 똑같은 말을 하다.", s: "빵을 다 꺼내고 나자 빵들이 입을 모아 말했습니다." },
        { w: "하마터면", k: "조금만 잘못했으면.", s: "\"고마워요. 하마터면 새까맣게 탈 뻔했어요.\"" },
        { w: "가지런히", k: "줄이 맞게 나란히.", s: "아이는 손을 호호 불면서 빵을 가지런히 늘어놓았습니다." },
        { w: "내리쬐다", k: "햇빛이 세게 내리비치다.", s: "들판에는 해가 따뜻하게 내리쬐었습니다." }
    ],
    "04-apples.webp": [
        { w: "열매", k: "나무에 달리는, 먹을 수 있는 알.", s: "\"저를 좀 흔들어 주세요. 열매가 너무 무거워서 가지가 부러지겠어요.\"" },
        { w: "주렁주렁", k: "열매가 많이 매달려 있는 모습.", s: "가지마다 빨간 사과가 주렁주렁 달려 있었습니다." },
        { w: "뻐근하다", k: "몸이 쑤시듯 무겁고 아프다.", s: "팔이 뻐근했지만 그만두지 않았지요." }
    ],
    "04-apples-2.webp": [
        { w: "홀가분하다", k: "짐을 벗은 듯 가볍고 편하다.", s: "나무가 홀가분한 듯 가지를 폈지요." },
        { w: "잎사귀", k: "나무에 달린 잎 한 장.", s: "잎사귀가 사르르 흔들렸습니다." },
        { w: "아삭아삭", k: "단단한 것을 시원하게 씹는 소리.", s: "아삭아삭 씹으니 참 달았지요." }
    ],
    "05-holle.webp": [
        { w: "이르다", k: "가서 닿다. 도착하다.", s: "이윽고 아이는 작은 집 앞에 이르렀습니다." },
        { w: "문가", k: "문이 있는 자리 언저리.", s: "문가에 할머니 한 사람이 서 있었지요." },
        { w: "껄껄", k: "크게 소리 내어 웃는 모양.", s: "그러자 할머니가 껄껄 웃었지요." }
    ],
    "05-holle-2.webp": [
        { w: "넉넉히", k: "모자라지 않고 넘칠 만큼.", s: "홀레 할머니는 밥을 넉넉히 차려 주었습니다." },
        { w: "흩날리다", k: "바람에 이리저리 날아 흩어지다.", s: "깃털이 눈송이처럼 흩날렸지요." },
        { w: "신기하다", k: "놀랍고 이상해서 재미있다.", s: "아이는 그것이 참 신기했지요." }
    ],
    "06-gold.webp": [
        { w: "부지런히", k: "쉬지 않고 열심히.", s: "아이는 날마다 부지런히 일했습니다." },
        { w: "칭찬", k: "잘했다고 좋게 말해 주는 일.", s: "오히려 잘한다고 칭찬해 주었습니다." },
        { w: "야무지다", k: "하는 일이 빈틈없고 단단하다.", s: "\"네 손끝이 참 야무지구나.\"" },
        { w: "그립다", k: "보고 싶어 마음이 끌리다.", s: "\"할머니, 저는 집이 그리워요.\"" }
    ],
    "06-gold-2.webp": [
        { w: "머리끝부터 발끝까지", k: "온몸 전부.", s: "금이 머리끝부터 발끝까지 붙었습니다." },
        { w: "건네주다", k: "손으로 넘겨 주다.", s: "할머니가 물레가락을 건네주었지요." },
        { w: "눈에 익다", k: "여러 번 보아 낯설지 않다.", s: "눈에 익은 마당이었지요." }
    ],
    "07-lazy.webp": [
        { w: "게으르다", k: "일하기 싫어하고 몸을 움직이지 않다.", s: "게으른 딸은 우물가에 앉았습니다." },
        { w: "손도 대지 않다", k: "아예 만지지도 않다.", s: "물레는 손도 대지 않았습니다." },
        { w: "가시", k: "찌를 만큼 뾰족한 것.", s: "그 대신 일한 척하려고 가시로 손끝을 찔러 피를 냈지요." },
        { w: "질끈", k: "눈을 힘주어 꽉 감는 모습.", s: "그러고는 눈을 질끈 감고 저도 뛰어들었지요." }
    ],
    "07-lazy-2.webp": [
        { w: "지나치다", k: "그냥 지나서 가 버리다.", s: "딸은 그냥 지나쳤고, 빵은 새까맣게 타 버렸습니다." },
        { w: "코웃음을 치다", k: "남을 우습게 보고 비웃다.", s: "딸은 코웃음을 쳤지요." },
        { w: "콧노래", k: "입을 닫고 코로 내는 노래.", s: "그러고는 콧노래를 부르며 걸어갔습니다." }
    ],
    "08-ending.webp": [
        { w: "냉큼", k: "망설이지 않고 얼른.", s: "금을 받고 싶어서 일하겠다고 냉큼 대답했지요." },
        { w: "오다 말다 하다", k: "이어지지 않고 끊겼다 이어지다.", s: "아래 세상에는 눈이 오다 말다 했습니다." },
        { w: "먼지가 앉다", k: "먼지가 내려와 덮이다.", s: "그릇은 쌓이고 방바닥에는 먼지가 앉았지요." },
        { w: "올 것이 왔다", k: "기다린 일이 드디어 일어났다.", s: "딸은 드디어 올 것이 왔다며 좋아했지요." }
    ],
    "08-ending-2.webp": [
        { w: "차례", k: "돌아오는 순서.", s: "이제 금이 쏟아질 차례라고 생각했지요." },
        { w: "시커멓다", k: "매우 검다.", s: "시커먼 송진이 주르륵 흘렀지요." },
        { w: "수탉", k: "닭의 수컷.", s: "수탉이 그 꼴을 보고 크게 울었지요." }
    ],
    "end.webp": [
        { w: "물레가락", k: "실을 감을 때 쓰는 뾰족한 막대.", s: "부지런한 아이는 우물에 빠진 물레가락을 주우러 뛰어듭니다." },
        { w: "제 손으로", k: "남을 시키지 않고 자기가 직접.", s: "잃어버린 것을 제 손으로 찾으려 한 것입니다." },
        { w: "말을 걸다", k: "먼저 이야기를 꺼내어 붙이다.", s: "빵과 사과나무가 말을 겁니다." },
        { w: "맡기다", k: "어떤 일을 하라고 남에게 넘겨주다.", s: "홀레 할머니는 두 아이에게 똑같이 이불 터는 일을 맡겼습니다." }
    ]
};

const UI = {
    ko: {
        toc: '차례', quiz: '이야기 문제', after: '읽고 나서',
        home: '학습 허브로 돌아가기', other: 'EN', otherAria: 'Read in English',
        page: n => `${n}쪽`, wordsDown: '낱말 ⌄',
        done: (n, all) => `${n} / 총 ${all}문항 완료`
    },
    en: {
        toc: 'Contents', quiz: 'Story Questions', after: 'After Reading',
        home: 'Back to the learning hub', other: '한국어', otherAria: '한국어로 읽기',
        page: n => `p. ${n}`, wordsDown: 'Words ⌄',
        done: (n, all) => `${n} of ${all} answered`
    }
};

const LANG_KEY = 'world-tales-lang';
const HAS_EN = typeof EN !== 'undefined';
const readLang = () => { try { return localStorage.getItem(LANG_KEY); } catch (e) { return null; } };
const saveLang = v => { try { localStorage.setItem(LANG_KEY, v); } catch (e) { /* 저장이 막힌 곳도 있다 */ } };

let LANG = (HAS_EN && readLang() === 'en') ? 'en' : 'ko';

const T  = () => UI[LANG];
const CH = () => (LANG === 'en' ? EN.chapters  : CHAPTERS);
const QZ = () => (LANG === 'en' ? EN.quiz      : QUIZ);
const AF = () => (LANG === 'en' ? EN.afterword : AFTERWORD);
const CV = () => (LANG === 'en' ? EN.cover     : COVER);

const TWO_PAGE_KINDS = new Set(['spread', 'toc', 'cover', 'after']);

/* 말을 바꾸면 쪽을 다시 짠다. 쪽 수는 두 말이 같으므로 보던 자리가 흔들리지 않는다. */
let PAGES = [];
let FOLIOS = [];

function buildPages() {
    const chs = CH();
    PAGES = [
    { kind: 'cover' },
    { kind: 'toc' },
    ...chs.flatMap((chapter, ci) => chapter.beats.map((beat, i) => ({
        kind: 'spread', chapter, beat,
        isFirst: i === 0,
        isLast: ci === chs.length - 1 && i === chapter.beats.length - 1
    }))),
    { kind: 'quiz' },
    ...AF().spreads.map((spread, i) => ({ kind: 'after', spread, isFirst: i === 0 }))
    ];

    let folioCounter = 0;
    FOLIOS = PAGES.map(p => {
        const width = TWO_PAGE_KINDS.has(p.kind) ? 2 : 1;
        const start = folioCounter + 1;
        folioCounter += width;
        return { start, width };
    });
}

function renderPage(page) {
    switch (page.kind) {
        case 'cover':
            return coverPage();
        case 'toc':
            return tocPage();
        case 'spread':
            return spreadPage(page.chapter, page.beat, page.isFirst, page.isLast);
        case 'after':
            return afterPage(page.spread, page.isFirst);
        case 'quiz':
            return quizPage();
        default:
            return '';
    }
}

let current = 0;
let animating = false;

const spreadEl = document.getElementById('spread');
const prevBtn = document.getElementById('prevBtn');
const nextBtn = document.getElementById('nextBtn');
const indicatorEl = document.getElementById('pageIndicator');
const folioLeftEl = document.getElementById('folioLeft');
const folioRightEl = document.getElementById('folioRight');

function paint() {
    spreadEl.innerHTML = renderPage(PAGES[current]);
    prevBtn.disabled = current === 0;
    nextBtn.disabled = current === PAGES.length - 1;
    indicatorEl.textContent = `${current + 1} / ${PAGES.length}`;

    const folio = FOLIOS[current];
    folioLeftEl.classList.toggle('folio-center', folio.width === 1);
    if (folio.width === 2) {
        folioLeftEl.textContent = folio.start;
        folioRightEl.textContent = folio.start + 1;
        folioLeftEl.hidden = false;
        folioRightEl.hidden = false;
    } else {
        folioLeftEl.textContent = folio.start;
        folioLeftEl.hidden = false;
        folioRightEl.hidden = true;
    }

    spreadEl.querySelectorAll('[data-goto]').forEach(btn => {
        btn.addEventListener('click', () => {
            const chapterNum = Number(btn.dataset.goto);
            const idx = PAGES.findIndex(p => p.kind === 'spread' && p.chapter.num === chapterNum && p.isFirst);
            if (idx >= 0) goTo(idx);
        });
    });
    spreadEl.querySelectorAll('[data-goto-kind]').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = PAGES.findIndex(p => p.kind === btn.dataset.gotoKind);
            if (idx >= 0) goTo(idx);
        });
    });

    if (PAGES[current].kind === 'quiz') {
        initQuiz();
    }

    const readBtn = document.getElementById('readBtn');
    if (readBtn) readBtn.addEventListener('click', () => (reading ? stopReading() : readPage(0)));

    // 읽는 중일 때만 문단을 눌러 그 자리로 옮긴다.
    if (LANG === 'en' && CAN_SPEAK) {
        spreadEl.querySelectorAll('[data-say]').forEach(el => {
            el.addEventListener('click', () => {
                if (!reading) return;
                readPage(Number(el.dataset.say));
            });
        });
    }
    if (typeof renderVocab === 'function') renderVocab();
}

function initQuiz() {
    const progressEl = document.getElementById('quizProgress');

    spreadEl.querySelectorAll('.quiz-item').forEach(item => {
        const qi = Number(item.dataset.qindex);
        const q = QZ()[qi];
        item.querySelectorAll('.quiz-choice').forEach(btn => {
            btn.addEventListener('click', () => {
                if (item.classList.contains('graded')) return;
                const ci = Number(btn.dataset.choice);
                // 맞힐 때까지 다시 고를 수 있다. 틀린 것만 빨갛게 남는다.
                // 답을 미리 보여 주면 아이가 생각할 자리가 사라진다.
                if (ci !== q.answer) {
                    btn.classList.add('incorrect');
                    (QUIZ_WRONG[QK(qi)] = QUIZ_WRONG[QK(qi)] || []).push(ci);
                    return;
                }
                btn.classList.add('correct');
                item.classList.add('graded');
                QUIZ_DONE[QK(qi)] = true;
                progressEl.textContent = T().done(quizDone(), QZ().length);
            });
        });
    });
}

function goTo(index) {
    if (animating || index === current || index < 0 || index >= PAGES.length) return;
    stopReading();
    animating = true;
    const dir = index > current ? 'flip-next' : 'flip-prev';
    spreadEl.classList.add(dir);
    setTimeout(() => {
        current = index;
        paint();
    }, 230);
    setTimeout(() => {
        spreadEl.classList.remove('flip-next', 'flip-prev');
        animating = false;
    }, 480);
}

prevBtn.addEventListener('click', () => goTo(current - 1));
nextBtn.addEventListener('click', () => goTo(current + 1));

document.getElementById('tocLink').addEventListener('click', () => {
    const idx = PAGES.findIndex(p => p.kind === 'toc');
    if (idx >= 0) goTo(idx);
});

document.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowRight') goTo(current + 1);
    if (e.key === 'ArrowLeft') goTo(current - 1);
});

/* ── 목소리 ────────────────────────────────────────────────────
   기기에 있는 영어 목소리를 골라 쓴다. 없으면 기본 목소리로 읽는다. */
const textOf = p => (typeof p === 'string' ? p : p.t);

/* 읽기 단추 — 표지·펼침면·읽고 나서에 붙는다. 영어로 볼 때만 나온다. */
function readBtnHtml() {
    return (LANG === 'en' && CAN_SPEAK)
        ? `<button type="button" class="read-btn" id="readBtn">${reading ? '■' : '▶'}</button>`
        : '';
}

/* 그 쪽에서 읽어 줄 글. 화면에 보이는 차례 그대로다. */
function pageParts(page) {
    if (!page) return [];
    if (page.kind === 'spread') return page.beat.left.concat(page.beat.right);
    if (page.kind === 'cover') return [CV().title].concat(CV().intro);
    if (page.kind === 'after') return page.spread.left.concat(page.spread.right);
    return [];
}

const SAY_RATE = 0.85;
const VOICES = { any: null };

function pickVoices() {
    if (typeof speechSynthesis === 'undefined') return;
    const vs = speechSynthesis.getVoices().filter(v => /^en/i.test(v.lang));
    if (!vs.length) return;
    VOICES.any = vs.find(v => /^en[-_]US/i.test(v.lang)) || vs[0];
}

if (typeof speechSynthesis !== 'undefined') {
    pickVoices();
    speechSynthesis.onvoiceschanged = pickVoices;
}

function dressVoice(u) {
    u.rate = SAY_RATE;
    if (VOICES.any) u.voice = VOICES.any;
}

/* ── 읽어 주기 ─────────────────────────────────────────────────
   펼침면 글을 왼쪽부터 차례로 읽는다. 읽는 문단에 표시가 따라간다.
   다시 누르면 멈춘다. 쪽을 넘기거나 우리말로 돌아가면 저절로 멈춘다. */
let reading = false;
let readToken = 0;

function stopReading() {
    reading = false;
    if (typeof spreadEl !== 'undefined' && spreadEl) spreadEl.classList.remove('is-reading');
    readToken++;
    if (CAN_SPEAK) { try { speechSynthesis.cancel(); } catch (e) {} }
    document.querySelectorAll('.saying').forEach(el => el.classList.remove('saying'));
    const b = document.getElementById('readBtn');
    if (b) b.textContent = '▶';
}

function readPage(from) {
    const page = PAGES[current];
    if (!CAN_SPEAK || !page) return;
    const parts = pageParts(page);
    if (!parts.length) return;
    try { speechSynthesis.cancel(); } catch (e) {}
    reading = true;
    if (spreadEl) spreadEl.classList.add('is-reading');
    const mine = ++readToken;
    const btn = document.getElementById('readBtn');
    if (btn) btn.textContent = '■';

    const step = (i) => {
        if (mine !== readToken) return;
        document.querySelectorAll('.saying').forEach(el => el.classList.remove('saying'));
        if (i >= parts.length) { stopReading(); return; }
        const here = document.querySelector(`[data-say="${i}"]`);
        if (here) {
            here.classList.add('saying');
            here.scrollIntoView({ block: 'nearest' });
        }
        const u = new SpeechSynthesisUtterance(textOf(parts[i]).replace(/"/g, ''));
        u.lang = 'en-US';
        dressVoice(u);
        u.onend = () => step(i + 1);
        u.onerror = () => step(i + 1);
        try { speechSynthesis.speak(u); } catch (e) { stopReading(); }
    };
    step(Math.max(0, Math.min(from | 0, parts.length - 1)));
}

/* ── 단어장 — 영어로 읽을 때만 책 아래에 깔린다 ──────────────────── */
const vocabScreenEl = document.getElementById('vocabScreen');
const vocabPanelEl = document.getElementById('vocabPanel');
const scrollDownEl = document.getElementById('scrollDown');
const HAS_WORDS = HAS_EN && EN.words && Object.keys(EN.words).length > 0;
const HAS_WORDS_KO = typeof WORDS_KO !== 'undefined' && WORDS_KO && Object.keys(WORDS_KO).length > 0;

/* 듣기 — 낱말을 먼저 읽고, 이어서 그 낱말이 나온 구절을 읽는다.
   목소리가 없는 기기에서는 단추 자체가 뜨지 않는다. */
const CAN_SPEAK = typeof speechSynthesis !== 'undefined';
let VOCAB_NOW = [];

function sayWord(item) {
    if (!CAN_SPEAK || !item) return;
    try {
        speechSynthesis.cancel();
        const bare = String(wOf(item)).replace(/\s*\([^)]*\)/g, '').trim();
        const 말 = LANG === 'en' ? 'en-US' : 'ko-KR';
        const word = new SpeechSynthesisUtterance(bare);
        word.lang = 말;
        word.rate = 0.75;
        const sent = new SpeechSynthesisUtterance(sOf(item));
        sent.lang = 말;
        speechSynthesis.speak(word);
        speechSynthesis.speak(sent);
    } catch (e) { /* 소리를 못 내는 기기도 있다 */ }
}

function vocabFor() {
    const all = LANG === 'en' ? ((HAS_WORDS && EN.words) || {}) : ((HAS_WORDS_KO && WORDS_KO) || {});
    const page = PAGES[current];
    // 그 쪽에 실제로 있는 글의 낱말을, 글에 나온 차례대로 보여 준다.
    const key = !page ? null
        : page.kind === 'spread' ? page.beat.art
        : page.kind === 'after' ? page.spread.art
        : null;
    if (key && all[key]) return all[key];
    // 표지에도 소개글이 있다. 그 글에 나온 낱말만 보여 준다.
    if (page && page.kind === 'cover') return all['cover.webp'] || all['cover'] || [];
    // 차례·문제 쪽에는 글이 없다. 보여 줄 것이 없으면 칸을 접는다.
    return [];
}

/* 낱말 자료는 우리말과 영어가 적는 이름이 다르다. 우리말 쪽은 전래동화와 같은
   {w, k, s}, 영어 쪽은 {word, meaning, sentence}다. 어느 쪽이 와도 읽게 한다. */
const wOf = w => (w.word !== undefined ? w.word : w.w);
const mOf = w => (w.meaning !== undefined ? w.meaning : w.k);
const sOf = w => (w.sentence !== undefined ? w.sentence : w.s);

function renderVocab() {
    const has = LANG === 'en' ? HAS_WORDS : HAS_WORDS_KO;
    const list = has ? vocabFor() : [];
    const on = list.length > 0;
    if (vocabScreenEl) vocabScreenEl.hidden = !on;
    if (scrollDownEl) {
        scrollDownEl.hidden = !on;
        scrollDownEl.textContent = T().wordsDown || (LANG === 'en' ? 'Words ⌄' : '낱말 ⌄');
    }
    if (!on) {
        if (window.scrollY) window.scrollTo({ top: 0 });
        return;
    }
    VOCAB_NOW = list;
    vocabPanelEl.innerHTML = `
        <ul class="vocab-list">
            ${list.map((w, i) => `
            <li>
                <div class="vocab-top">
                    <p class="vocab-word">${wOf(w)}</p>
                    ${(CAN_SPEAK && LANG === 'en') ? `<button type="button" class="vocab-say" data-i="${i}" aria-label="Listen">🔊</button>` : ''}
                </div>
                <p class="vocab-mean">${mOf(w)}</p>
                <p class="vocab-sent">${sOf(w)}</p>
            </li>`).join('')}
        </ul>`;
    fitVocabScreen();
}

if (vocabPanelEl) {
    vocabPanelEl.addEventListener('click', e => {
        const btn = e.target.closest('.vocab-say');
        if (btn) sayWord(VOCAB_NOW[Number(btn.dataset.i)]);
    });
}

/* 그림선 — 그림 칸이 끝나는 자리다. 여기까지만 내려가면 그림만 위로 빠지고
   읽던 글은 화면에 남는다. 아래 화면 높이를 딱 그만큼 주면 더 내려갈 데가
   없어져 저절로 그 자리에 걸린다. */
function artLine() {
    const page = PAGES[current];
    const el = !page ? null
        : page.kind === 'spread' ? document.querySelector('.spread-art')
        : page.kind === 'cover' ? document.querySelector('.page-cover .story-page-left-full')
        : page.kind === 'after' ? document.querySelector('.after-art')
        : null;
    if (!el) return 0;
    const box = el.getBoundingClientRect();
    const line = box.bottom + window.scrollY;
    const book = document.querySelector('.book');
    if (!book) return Math.max(0, Math.round(line));
    const bookBox = book.getBoundingClientRect();
    // 그림이 책 높이를 거의 다 차지하면 경계선이 곧 책 밑이라 책이 통째로
    // 사라진다. 그때만 책이 절반쯤 남도록 붙든다.
    if (box.height < bookBox.height * 0.8) return Math.max(0, Math.round(line));
    const cap = bookBox.bottom + window.scrollY - Math.round(window.innerHeight * 0.45);
    return Math.max(0, Math.round(Math.min(line, Math.max(cap, 0))));
}

function fitVocabScreen() {
    if (!vocabScreenEl || vocabScreenEl.hidden) return;
    const line = artLine();
    // 펼침면이 아닌 쪽(차례·문제)에는 그림 칸이 없다. 그때는 내용만큼만 둔다.
    vocabScreenEl.style.minHeight = line ? line + 'px' : '';
}

if (scrollDownEl) {
    scrollDownEl.addEventListener('click', () => {
        fitVocabScreen();
        const line = artLine();
        window.scrollTo({ top: line || document.documentElement.scrollHeight, behavior: 'smooth' });
    });
}

window.addEventListener('resize', () => { window.scrollTo(0, 0); fitVocabScreen(); });

/* 말 바꾸기 — 보던 자리를 그대로 두고 글만 갈아 끼운다. */
const langBtn = document.getElementById('langLink');
function applyLang() {
    if (typeof stopReading === 'function') stopReading();
    document.documentElement.lang = LANG;
    document.title = CV().title;
    if (langBtn) {
        langBtn.hidden = !HAS_EN;
        langBtn.textContent = T().other;
        langBtn.setAttribute('aria-label', T().otherAria);
    }
}
if (langBtn && HAS_EN) {
    langBtn.addEventListener('click', () => {
        LANG = LANG === 'en' ? 'ko' : 'en';
        saveLang(LANG);
        const here = PAGES[current];
        buildPages();
        current = Math.min(current, PAGES.length - 1);
        // 보던 그림 그대로 선다. 쪽 수가 같으므로 그림 파일 이름으로 찾는다.
        if (here && here.kind === 'spread') {
            const i = PAGES.findIndex(p => p.kind === 'spread' && p.beat.art === here.beat.art);
            if (i >= 0) current = i;
        }
        applyLang();
        paint();
    });
}

buildPages();
applyLang();
paint();

