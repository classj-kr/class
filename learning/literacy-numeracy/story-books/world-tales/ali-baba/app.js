const CHAPTERS = [
    {
        "num": 1,
        "title": "1장 · 열려라 참깨",
        "beats": [
            {
                "art": "01-cave.webp",
                "emoji": "🚪",
                "left": [
                    "도끼 한 자루와 당나귀 세 마리.",
                    "알리바바의 살림은 그것이 전부였습니다. 형 카심은 같은 마을에서 제일가는 부자였지요. 형제라도 사는 모습이 아주 달랐습니다.",
                    "그날도 알리바바는 새벽에 산으로 나무를 하러 갔지요. 한창 도끼질을 하는데 멀리서 먼지가 뽀얗게 일었습니다."
                ],
                "right": [
                    "두두두두.",
                    "말발굽 소리가 땅을 울리며 점점 가까워졌지요. 알리바바는 얼른 나무 위로 기어올랐습니다.",
                    "숨을 죽이고 내려다보니 말 탄 사람들이 몰려오고 있었지요."
                ]
            },
            {
                "art": "01-cave-2.webp",
                "emoji": "🚪",
                "left": [
                    "알리바바는 손가락을 꼽아 수를 세었습니다. 모두 마흔 명이었지요. 하나같이 천으로 얼굴을 가리고 있었습니다.",
                    "등에는 묵직한 자루를 하나씩 지고 있었지요.",
                    "이윽고 두목으로 보이는 사내가 앞으로 나섰습니다. 사내는 커다란 바위 앞에 우뚝 섰지요."
                ],
                "right": [
                    "\"열려라, 참깨!\"",
                    "그러자 바위가 쩍 하고 갈라졌습니다. 도적들이 줄줄이 안으로 들어가더니, 한참 뒤에 다들 빈손으로 나왔지요.",
                    "두목이 다시 외쳤습니다. \"닫혀라, 참깨!\"",
                    "그러자 바위가 감쪽같이 다시 붙었지요."
                ]
            }
        ]
    },
    {
        "num": 2,
        "title": "2장 · 보물이 가득한 동굴",
        "beats": [
            {
                "art": "02-treasure.webp",
                "emoji": "💰",
                "left": [
                    "도적들이 멀리 사라진 뒤였습니다. 알리바바는 조심조심 나무에서 내려와 한참을 망설였지요.",
                    "그러다 큰맘 먹고 바위 앞에 섰습니다. 아까 들은 주문을 입속으로 몇 번이나 중얼거려 보았지요.",
                    "\"여, 열려라 참깨.\"",
                    "그러자 바위가 정말로 갈라졌습니다."
                ],
                "right": [
                    "안으로 들어선 알리바바는 눈이 부셨지요. 금화가 산처럼 쌓여 있었습니다.",
                    "비단이며 보석도 가득했지요. 알리바바는 눈을 몇 번이나 비볐습니다.",
                    "\"이걸 다 어디서 가져왔을까.\"",
                    "한참을 그 자리에 서 있었지요."
                ]
            },
            {
                "art": "02-treasure-2.webp",
                "emoji": "💰",
                "left": [
                    "알리바바는 금화만 조금 담기로 했습니다. 자루 세 개에 나눠 담아 당나귀 등에 실었지요. 그 위에는 나뭇짐을 수북이 덮었습니다.",
                    "\"닫혀라, 참깨.\"",
                    "바위가 다시 붙었지요. 알리바바는 뒤도 돌아보지 않고 산을 내려왔습니다."
                ],
                "right": [
                    "알리바바가 자루를 풀자 아내는 눈이 휘둥그레졌습니다. \"이걸 다 세어 봐야겠어요.\"",
                    "알리바바가 손을 저었지요. \"세지 말고 그냥 묻어 둡시다.\"",
                    "그래도 아내는 궁금해 참지 못했습니다. 하나하나 세기엔 너무 많아, 곡식 양을 재는 나무 그릇인 됫박으로 재어 보기로 했지요. 그래서 형네 집으로 됫박을 빌리러 갔습니다."
                ]
            }
        ]
    },
    {
        "num": 3,
        "title": "3장 · 형의 욕심",
        "beats": [
            {
                "art": "03-brother.webp",
                "emoji": "🪙",
                "left": [
                    "카심의 아내는 됫박을 빌려주면서 고개를 갸웃했습니다. \"가난한 집이 대체 뭘 잰다는 거지?\"",
                    "그래서 됫박 바닥에 밀랍을 몰래 발라 두었지요. 밀랍은 벌집에서 나온 것인데, 뭐든 척척 들러붙습니다.",
                    "돌려받은 됫박을 뒤집어 보니 밀랍에 금화 한 닢이 딱 붙어 있었지요. 카심의 아내는 그길로 남편에게 달려갔습니다."
                ],
                "right": [
                    "카심은 당장 동생을 찾아가 다그쳤습니다. \"이 금이 대체 어디서 났느냐!\"",
                    "알리바바는 끝내 숨기지 못하고 있었던 일을 다 말했지요. 카심의 눈이 번쩍 뜨였습니다. \"그 동굴이 어디냐? 당장 대라.\"",
                    "알리바바가 말렸지요. \"형님, 욕심내면 안 됩니다.\" 그러나 카심은 들은 척도 하지 않았습니다."
                ]
            },
            {
                "art": "03-brother-2.webp",
                "emoji": "🪙",
                "left": [
                    "다음 날 새벽 카심이 산으로 갔습니다. 노새를 열 마리나 끌고 갔지요.",
                    "\"열려라, 참깨!\"",
                    "바위가 쩍 갈라졌습니다. 안으로 들어간 카심은 그만 정신이 나갔지요. 자루마다 금을 그득그득 퍼 담았습니다."
                ],
                "right": [
                    "자루를 다 채운 카심은 싱글벙글했습니다. 그런데 막상 나가려니 주문이 떠오르지 않았지요.",
                    "\"열려라, 보리! 열려라, 콩!\"",
                    "생각나는 곡식 이름을 다 대 보았지만 바위는 꿈쩍도 하지 않았습니다. 그러는 사이 도적들이 돌아오고 말았지요."
                ]
            }
        ]
    },
    {
        "num": 4,
        "title": "4장 · 표시된 대문",
        "beats": [
            {
                "art": "04-chalk.webp",
                "emoji": "✖️",
                "left": [
                    "카심은 밤이 깊도록 돌아오지 않았습니다. 형수가 발을 동동 구르며 알리바바를 찾아왔지요.",
                    "이튿날 알리바바가 동굴로 가 보았습니다. 형은 도적들에게 들켜 이미 목숨을 잃은 뒤였지요. 알리바바는 형을 당나귀에 싣고 몰래 집으로 모셔 왔습니다.",
                    "형수는 그 자리에 주저앉아 울었지요. 그때 형네 집 하녀 모르지아나가 나섰습니다."
                ],
                "right": [
                    "모르지아나는 눈치가 아주 빨랐지요. \"소문이 나면 도적들이 이 집을 찾아올 거예요. 제가 알아서 하겠어요.\"",
                    "모르지아나는 주인님이 병으로 돌아가셨다고 이웃에 알렸습니다. 그러고는 조용히 장례를 치렀지요. 동굴 일은 아무도 눈치채지 못했습니다.",
                    "그 뒤 알리바바네 식구는 형네 집으로 옮겨 와 함께 살았지요."
                ]
            },
            {
                "art": "04-chalk-2.webp",
                "emoji": "✖️",
                "left": [
                    "한편 도적들은 동굴에서 카심이 사라진 것을 알았습니다. 두목이 이를 부드득 갈았지요. \"동굴을 아는 자가 또 있구나.\"",
                    "도적 하나가 마을로 내려갔습니다. 요즘 장례를 치른 집이 있는지 이 집 저 집 물어보고 다녔지요.",
                    "드디어 그 집을 알아냈습니다. 도적은 대문에 분필로 슬쩍 표시를 해 두었지요."
                ],
                "right": [
                    "이튿날 아침 모르지아나가 물을 길러 나왔지요. 대문의 표시를 보고는 걸음을 뚝 멈췄습니다.",
                    "'누가 우리 집을 찾아내려고 표시를 했구나.'",
                    "모르지아나는 얼른 분필을 가져왔지요. 그러고는 온 동네 대문에 똑같은 표시를 그렸습니다. 스무 집, 서른 집이 되었지요."
                ]
            }
        ]
    },
    {
        "num": 5,
        "title": "5장 · 마당의 항아리 마흔 개",
        "beats": [
            {
                "art": "05-jars.webp",
                "emoji": "🏺",
                "left": [
                    "그날 밤 도적들은 헛걸음을 했습니다. 똑같은 표시가 온 동네 대문에 있었으니까요.",
                    "두목이 버럭 소리를 질렀지요. \"어느 집이란 말이냐!\"",
                    "결국 두목이 직접 마을로 내려갔습니다. 이번에는 표시를 하지 않았지요. 대신 집 모양을 눈에 꼭꼭 담아 두었습니다."
                ],
                "right": [
                    "며칠 뒤 알리바바의 집에 손님이 찾아왔습니다. 기름 장수 차림이었지요.",
                    "\"하룻밤만 묵게 해 주십시오.\"",
                    "알리바바는 선뜻 방을 내주었습니다. 손님은 싣고 온 큰 항아리를 마당에 부려 놓았지요. 모두 마흔 개였습니다."
                ]
            },
            {
                "art": "05-jars-2.webp",
                "emoji": "🏺",
                "left": [
                    "밤이 깊었습니다. 모르지아나가 등잔에 불을 켜려는데 기름이 떨어져 있었지요.",
                    "\"마당 항아리에서 좀 덜어 오자.\"",
                    "모르지아나가 항아리에 다가가자, 안에서 소곤소곤 소리가 났지요. \"두목님, 지금입니까?\""
                ],
                "right": [
                    "모르지아나는 숨이 딱 멎었습니다. 그래도 목소리를 굵게 낮춰 대답했지요. \"아직이다. 기다려라.\"",
                    "그러자 항아리 속에서 대답이 들렸습니다. \"예, 알겠습니다.\"",
                    "모르지아나는 항아리를 하나하나 지나가 보았지요. 가는 곳마다 똑같이 묻는 소리가 났습니다. 기름이 든 항아리는 딱 하나뿐이었지요."
                ]
            }
        ]
    },
    {
        "num": 6,
        "title": "6장 · 모르지아나의 춤",
        "beats": [
            {
                "art": "06-dance.webp",
                "emoji": "💃",
                "left": [
                    "모르지아나는 서두르지 않았습니다. 기름이 든 항아리로 가서 기름을 크게 한 솥 펐지요. 그러고는 부엌에서 팔팔 끓였습니다.",
                    "부글부글.",
                    "김이 천장까지 올라갔지요. 모르지아나는 그 솥을 들고 마당으로 나왔습니다."
                ],
                "right": [
                    "그러고는 항아리마다 뜨거운 기름을 부었지요. 안에서 소란이 일었습니다.",
                    "\"으악, 뜨거워!\"",
                    "도적들이 항아리에서 뛰쳐나와 껑충껑충 뛰었지요. 그러고는 담을 넘어 뿔뿔이 달아났습니다. 방에 있던 두목도 그 소리에 놀라 뒷문으로 빠져나갔지요."
                ]
            },
            {
                "art": "06-dance-2.webp",
                "emoji": "💃",
                "left": [
                    "얼마 뒤 두목이 또 찾아왔습니다. 이번에는 장사꾼 차림이었지요. 두목은 알리바바의 아들과 슬슬 친해지더니, 저녁 초대까지 받았습니다.",
                    "그런데 상을 차리던 모르지아나가 손님 얼굴을 보았지요.",
                    "'어디서 본 얼굴인데.'"
                ],
                "right": [
                    "모르지아나는 곰곰이 생각하다 무릎을 쳤지요. '그 기름 장수야!'",
                    "저녁을 먹고 나자 모르지아나는 춤을 추겠다고 나섰습니다. 허리에는 작은 단검을 차고 있었지요.",
                    "둥, 둥, 둥. 북소리에 맞춰 빙글빙글 돌던 춤이 손님 앞에서 뚝 멈췄습니다. 모르지아나는 단검 끝으로 손님의 옷자락을 휙 젖혔지요."
                ]
            }
        ]
    },
    {
        "num": 7,
        "title": "7장 · 정체가 드러나다",
        "beats": [
            {
                "art": "07-revealed.webp",
                "emoji": "🔦",
                "left": [
                    "그 순간 손님의 옷 안이 드러났습니다. 허리에 커다란 칼이 숨겨져 있었지요.",
                    "모르지아나가 소리쳤습니다. \"주인님, 이 칼을 보십시오! 지난번 기름 장수가 바로 이 사람입니다.\"",
                    "방 안이 순식간에 얼어붙었지요."
                ],
                "right": [
                    "아들이 들고 있던 잔을 떨어뜨렸습니다. 두목은 벌떡 일어나 문으로 달아나려 했지요.",
                    "그런데 문 앞에 사람들이 빽빽이 서 있었지요. 이웃들이 지키고 있었던 것입니다. 모르지아나가 춤을 추기 전에 미리 불러 둔 것이었지요.",
                    "두목은 그길로 붙잡히고 말았습니다."
                ]
            },
            {
                "art": "07-revealed-2.webp",
                "emoji": "🔦",
                "left": [
                    "알리바바는 한참 동안 말이 없다가 모르지아나를 바라봤지요. \"자네가 우리 집을 다 살렸네. 이제 자네는 종이 아닐세.\"",
                    "모르지아나는 수줍게 웃었습니다. \"저는 할 일을 했을 뿐입니다.\""
                ],
                "right": [
                    "얼마 뒤 알리바바는 다시 동굴로 갔습니다. 이번에는 아들과 함께였지요. 가져온 금은 마을 사람들에게 고루 나누었습니다.",
                    "그 돈으로 우물을 파고 무너진 길도 고쳤지요. 다만 주문만은 아무에게도 알리지 않았습니다. 그 집만 아는 말로 남았지요."
                ]
            }
        ],
        "moral": "뜻밖의 행운을 만나도 욕심을 부리면 위험해져요. 침착하게 살피는 눈이 결국 모두를 지켜 낸답니다."
    }
];

function artFrame(src, emoji) {
    return `
        <div class="art-frame">
            <img src="images/${src}?v=20260918" alt="" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
            <div class="art-fallback" style="display:none">${emoji}</div>
        </div>`;
}

/* 표지 글도 말에 따라 갈아 끼우므로 상수로 뺐다. */
const COVER = {
    title: '알리바바와 40인의 도적',
    intro: [
        '아라비안나이트, 곧 천일야화에 실린 이야기예요. 알라딘과 같은 책에서 나왔답니다.',
        '가난한 나무꾼 알리바바가 산에서 우연히 도둑 떼의 비밀을 엿보면서 이야기가 시작돼요. 그 비밀을 알게 된 알리바바네 집에는 어떤 일이 벌어질까요?'
    ]
};

function coverPage() {
    const cv = CV();
    return `
        <div class="page page-cover">
            <div class="story-page-left story-page-left-full">
                ${artFrame('cover.webp', '🗝️')}
                ${readBtnHtml()}
            </div>
            <div class="story-page-right">
                <h1 data-say="0">${cv.title}</h1>
                ${cv.intro.map((p, i) => `<p data-say="${i + 1}">${p}</p>`).join('')}
            </div>
        </div>`;
}

function tocPage() {
    const pageOf = num => {
        const idx = PAGES.findIndex(p => p.kind === 'spread' && p.chapter.num === num && p.isFirst);
        return idx >= 0 ? FOLIOS[idx].start : '';
    };
    const itemHtml = s => `
        <li>
            <button type="button" data-goto="${s.num}">
                <span class="toc-num">${s.num}</span>
                <span>
                    <strong>${s.title.replace(/^(\d+장|Chapter \d+) · /, '')}</strong>
                    <small>${T().page(pageOf(s.num))}</small>
                </span>
            </button>
        </li>`;
    const quizIdx = PAGES.findIndex(p => p.kind === 'quiz');
    const quizItemHtml = `
        <li>
            <button type="button" data-goto-kind="quiz">
                <span class="toc-num">❓</span>
                <span>
                    <strong>${T().quiz}</strong>
                    <small>${quizIdx >= 0 ? T().page(FOLIOS[quizIdx].start) : ''}</small>
                </span>
            </button>
        </li>`;
    const afterIdx = PAGES.findIndex(p => p.kind === 'after' && p.isFirst);
    const afterItemHtml = `
        <li>
            <button type="button" data-goto-kind="after">
                <span class="toc-num">📖</span>
                <span>
                    <strong>${T().after}</strong>
                    <small>${afterIdx >= 0 ? T().page(FOLIOS[afterIdx].start) : ''}</small>
                </span>
            </button>
        </li>`;
    const chs = CH();
    const half = Math.ceil(chs.length / 2);
    const leftItems = chs.slice(0, half).map(itemHtml).join('');
    const rightItems = chs.slice(half).map(itemHtml).join('') + quizItemHtml + afterItemHtml;
    return `
        <div class="page page-toc">
            <div class="story-page-left">
                <h2>${T().toc}</h2>
                <ul class="toc-list">${leftItems}</ul>
            </div>
            <div class="story-page-right">
                <h2 class="toc-h2-ghost" aria-hidden="true">${T().toc}</h2>
                <ul class="toc-list">${rightItems}</ul>
            </div>
        </div>`;
}

function spreadPage(chapter, beat, isFirst, isLast) {
    const badgeHtml = isFirst ? `<div class="spread-chapter-badge">${chapter.title}</div>` : '';
    const n = beat.left.length;
    const leftHtml = beat.left.map((p, i) => `<p data-say="${i}">${p}</p>`).join('');
    let rightHtml = beat.right.map((p, i) => `<p data-say="${n + i}">${p}</p>`).join('');
    // 교훈은 오른쪽 칸이 아니라 두 칸 아래 제 자리에 놓는다. 칸 안에 밀어 넣으면 넘친다.
    const moralHtml = isLast && chapter.moral ? `<p class="fable-moral">${chapter.moral}</p>` : '';
    return `
        <div class="page page-story${moralHtml ? ' spread-final' : ''}">
            <div class="spread-art">
                ${badgeHtml}
                ${artFrame(beat.art, beat.emoji)}
                ${readBtnHtml()}
            </div>
            <div class="spread-text">
                <div class="spread-text-left">${leftHtml}</div>
                <div class="spread-text-right">${rightHtml}</div>
                ${moralHtml}
            </div>
        </div>`;
}

/* 읽고 나서 — 소설 트랙과 같은 형식이다. 다만 동화틀은 쪽을 재서 나누지 않으므로
   왼쪽·오른쪽 글을 손으로 나누어 둔다. 끝 쪽은 그대로 두므로 학습 허브로 가는
   단추 자리를 여기서 뺄 필요가 없다. */
const AFTERWORD = {
    title: '읽고 나서',
    emoji: '🏺',
    spreads: [
        {
            art: 'end.webp',
            left: [
                "알리바바는 원래 아라비안나이트에 없었습니다. 삼백 년쯤 전 프랑스 사람이 옮기면서 새로 넣은 것이지요.",
                "알리바바는 부자가 아니었습니다. 나무를 해서 파는 사람이었지요. 도둑들이 여는 문을 본 것도 나무를 하러 갔기 때문입니다.",
                "'열려라 참깨'는 아주 짧습니다. 형은 그 짧은 말을 잊어버려 굴에 갇히지요. 알리바바는 입속으로 여러 번 중얼거려 외워 두었습니다.",
                "형이 굴을 알게 된 것은 형수 때문입니다. 됫박 바닥에 밀랍을 발라 두었더니 금화가 붙어 나왔고, 알리바바는 더 숨길 수 없었지요."
            ],
            right: [
                "이 이야기에서 가장 여러 번 사람을 살린 것은 하녀 모르지아나입니다. 항아리 속에서 나는 소리를 알아듣고, 분필 자국을 온 집에 똑같이 그려 놓고요.",
                "알리바바는 굴에서 얼마를 가져갔어야 할까요?"
            ]
        }
    ]
};

function afterPage(spread, isFirst) {
    const head = isFirst ? `<h2>${AF().title}</h2>` : '';
    // 그림은 오른쪽 위 모서리에 붙고, 글을 뺀 나머지 자리를 다 차지한다.
    const art = spread.art ? `<div class="after-art">${artFrame(spread.art, AF().emoji)}</div>` : '';
    const col = (ps, from) => ps.map((t, i) => `<p data-say="${from + i}">${t}</p>`).join('');
    return `
        <div class="page page-after">
            <div class="after-col after-col-left">
                ${head}
                ${col(spread.left, 0)}
            </div>
            <div class="after-col after-col-right${spread.art ? ' after-col-image' : ''}">
                ${art}
                ${readBtnHtml()}
                ${col(spread.right, spread.left.length)}
                <p class="after-home"><a class="home-btn" href="../../../../../">${T().home}</a></p>
            </div>
        </div>`;
}

const QUIZ = [
    {
        "q": "카심은 동굴에서 나오려고 무어라 외쳤나요?",
        "choices": [
            "열려라 참깨",
            "닫혀라 참깨",
            "열려라 보리"
        ],
        "answer": 2
    },
    {
        "q": "알리바바가 처음 동굴에서 가지고 나온 것은 무엇인가요?",
        "choices": [
            "금화 자루 셋",
            "비단과 보석 한 자루",
            "금화 한 닢"
        ],
        "answer": 0
    },
    {
        "q": "형수는 알리바바네에 금화가 있는 것을 어떻게 알았나요?",
        "choices": [
            "알리바바의 아내가 자랑을 해서",
            "됫박 바닥에 밀랍을 발라 두어서",
            "산으로 가는 알리바바를 뒤따라가서"
        ],
        "answer": 1
    },
    {
        "q": "도적은 카심의 집을 어떻게 찾아냈나요?",
        "choices": [
            "요즘 장례를 치른 집을 물어보고 다녔다",
            "됫박에 붙은 금화를 보고 알았다",
            "알리바바의 아들과 친해져 초대를 받았다"
        ],
        "answer": 0
    },
    {
        "q": "항아리 속에 도적들이 숨은 것을 안 모르지아나는 어떻게 했나요?",
        "choices": [
            "이웃들을 불러 항아리를 지키게 했다",
            "항아리마다 뜨거운 기름을 부었다",
            "두목이 자는 방문을 잠갔다"
        ],
        "answer": 1
    },
    {
        "q": "모르지아나는 춤을 추다가 손님 앞에서 무엇을 했나요?",
        "choices": [
            "단검으로 옷자락을 젖혀 숨긴 칼을 보였다",
            "잔을 떨어뜨려 이웃들에게 신호를 보냈다",
            "등잔불을 얼굴에 비추어 알아보게 했다"
        ],
        "answer": 0
    },
    {
        "q": "두목이 붙잡힌 뒤 알리바바는 모르지아나에게 무어라 말했나요?",
        "choices": [
            "이 집을 자네에게 주겠네",
            "이제 자네는 종이 아닐세",
            "금화를 반씩 나누세"
        ],
        "answer": 1
    },
    {
        "q": "이 책을 읽고 난 반응으로 알맞지 않은 것은 무엇인가요?",
        "wide": true,
        "choices": [
                "카심이 노새를 열 마리나 끌고 간 것을 보면, 동생 몫까지 가져다주려 했구나.",
                "알리바바가 세지 말고 그냥 묻어 두자고 한 것을 보면, 표가 날까 봐 걱정했네.",
                "모르지아나가 온 동네 대문에 똑같은 표시를 그린 것을 보면, 지우는 대신 쓸모없게 만들었어.",
                "모르지아나가 춤을 추기 전에 이웃들을 미리 불러 둔 것을 보면, 혼자 맞설 생각이 아니었구나."
            ],
        "answer": 0
    }
];

/* 푼 자취는 책을 덮을 때까지 남긴다. 쪽을 옮겼다 돌아와도 표시가 그대로다.
   자취는 보기의 원래 번호로 적어 두므로, 다시 그릴 때 자리가 바뀌어도 따라간다. */
/* 한글 문제와 영어 문제는 서로 다른 것을 묻는다. 그래서 자취도 말별로 따로 적는다.
   한 자리에 같이 적어 두면, 말을 바꿨을 때 아이가 누른 적 없는 보기에 표시가 앉는다. */
const QUIZ_WRONG = {};   // 말:문제 번호 → 잘못 고른 보기의 원래 번호들
const QUIZ_DONE = {};    // 말:문제 번호 → 맞혔는가
const QUIZ_ORDER = {};   // 말:문제 번호 → 보기를 늘어놓을 차례
const QK = i => LANG + ':' + i;
const quizDone = () => Object.keys(QUIZ_DONE).filter(k => k.startsWith(LANG + ':')).length;

/* 보기 차례는 책을 여는 순간 한 번만 정한다. 답의 자리를 외워 버리면 문제가 아니게 되니 섞되,
   쪽을 옮길 때마다 다시 섞으면 방금 고른 자리가 눈앞에서 움직여 어지럽다.
   책을 나갔다 들어오면 처음부터 다시 정해진다.
   섞는 것은 그리는 차례뿐이고, 채점은 data-choice에 담긴 원래 번호로 한다. */
function shuffledOrder(n) {
    const a = [...Array(n).keys()];
    for (let i = a.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [a[i], a[j]] = [a[j], a[i]];
    }
    return a;
}

function quizPage() {
    const items = QZ().map((item, i) => {
        const wrong = QUIZ_WRONG[QK(i)] || [];
        const solved = !!QUIZ_DONE[QK(i)];
        const order = QUIZ_ORDER[QK(i)] || (QUIZ_ORDER[QK(i)] = shuffledOrder(item.choices.length));
        const btns = order.map(ci => {
            const mark = solved && ci === item.answer ? ' correct' : (wrong.includes(ci) ? ' incorrect' : '');
            return `<button type="button" class="quiz-choice${mark}" data-choice="${ci}">${item.choices[ci]}</button>`;
        }).join('');
        return `
        <div class="quiz-item${solved ? ' graded' : ''}" data-qindex="${i}">
            <p class="quiz-question">${i + 1}. ${item.q}</p>
            <div class="quiz-choices${item.wide ? ' quiz-choices-stack' : ''}">${btns}</div>
        </div>`;
    }).join('');
    return `
        <div class="page page-quiz">
            <h2>${T().quiz}</h2>
            <p class="quiz-intro-text" id="quizProgress">${T().done(quizDone(), QZ().length)}</p>
            <div class="quiz-list">${items}</div>
        </div>`;
}


/* 영어판 — 한국어 원고를 줄 단위로 옮기지 않고 영어로 다시 썼다. */
const EN = {
    lang: 'en',
    cover: {
        title: 'Ali Baba and the Forty Thieves',
        intro: [
            "Also from the Arabian Nights, the thousand and one nights of tales.",
            "It begins when Ali Baba, a poor woodcutter, happens to see a band of robbers in the hills and learns their secret. What will happen to his household now that he knows it?"
        ]
    },
    chapters: [
        {
            num: 1,
            title: 'Chapter 1 · Open Sesame',
            beats: [
                {
                    art: '01-cave.webp',
                    emoji: '🪓',
                    left: [
                        "One axe and three donkeys.",
                        "That was everything Ali Baba owned. His brother Cassim was the richest man in the village.",
                        "That day, as usual, Ali Baba went up the hill at dawn to cut wood.",
                        "He was hard at work with his axe when a cloud of dust went up far away."
                    ],
                    right: [
                        "Clip-clop, clip-clop!",
                        "The sound of hooves came shaking the ground, closer and closer.",
                        "Ali Baba scrambled quickly up a tree.",
                        "He held his breath and looked down. A whole troop of riders was coming."
                    ]
                },
                {
                    art: '01-cave-2.webp',
                    emoji: '🪓',
                    left: [
                        "Ali Baba counted them on his fingers. There were forty of them.",
                        "Every one had a cloth over his face, and every one carried a heavy sack on his back.",
                        "Then a man who looked like the captain came forward and stood before a great rock."
                    ],
                    right: [
                        "\"Open, Sesame!\" the captain shouted.",
                        "And the rock split apart. The thieves went in one behind another. A long while later they all came out empty-handed.",
                        "The captain shouted again, \"Shut, Sesame!\"",
                        "And the rock closed up without a seam."
                    ]
                }
            ]
        },
        {
            num: 2,
            title: 'Chapter 2 · The Cave Full of Treasure',
            beats: [
                {
                    art: '02-treasure.webp',
                    emoji: '💰',
                    left: [
                        "When the thieves were gone, Ali Baba climbed carefully down.",
                        "He hung about behind the tree for a long while.",
                        "Then he screwed up his courage and stood before the rock, saying the words over to himself.",
                        "\"O-open, Sesame.\"",
                        "And the rock really did split apart."
                    ],
                    right: [
                        "Inside, the light dazzled him. Gold coins lay heaped up like hills, and there were silks and jewels besides.",
                        "Ali Baba rubbed his eyes over and over.",
                        "\"Wherever did they get all this?\"",
                        "He stood there a long time."
                    ]
                },
                {
                    art: '02-treasure-2.webp',
                    emoji: '💰',
                    left: [
                        "Ali Baba decided to take only a little of the gold. He divided it into three sacks, loaded them on the donkeys, and heaped firewood on top.",
                        "\"Shut, Sesame.\"",
                        "The rock closed again, and Ali Baba went down the hill without looking back."
                    ],
                    right: [
                        "At home he untied the sacks, and his wife's eyes went round. \"I must count all this!\"",
                        "Ali Baba waved his hand. \"Never mind counting. Let us bury it.\"",
                        "But she had to know. There was too much to count coin by coin, so she went to Cassim's house to borrow a measure, a wooden bowl for grain."
                    ]
                }
            ]
        },
        {
            num: 3,
            title: "Chapter 3 · The Brother's Greed",
            beats: [
                {
                    art: '03-brother.webp',
                    emoji: '🕯️',
                    left: [
                        "Cassim's wife thought it very odd. \"What can a poor house have to measure?\"",
                        "So she smeared a little wax on the bottom of the measure. Wax sticks to anything.",
                        "When the measure came back, she turned it over. A gold coin was stuck fast in the wax!",
                        "She ran straight to Cassim with it."
                    ],
                    right: [
                        "Cassim ran to his brother. \"Where did this gold come from?\"",
                        "Ali Baba could not hide it. He told the whole thing, leaving nothing out.",
                        "Cassim's eyes lit up. \"Where is that cave? Tell me now!\"",
                        "\"Brother, you must not be greedy about this,\" said Ali Baba. But Cassim would not listen."
                    ]
                },
                {
                    art: '03-brother-2.webp',
                    emoji: '🕯️',
                    left: [
                        "Next day at dawn Cassim went up the hill, and he took ten mules with him.",
                        "\"Open, Sesame!\"",
                        "The rock split apart.",
                        "Once inside, Cassim quite lost his head, and he filled sack after sack to the brim with gold."
                    ],
                    right: [
                        "\"That will do,\" said Cassim with a big grin.",
                        "But when he came to go out, he could not remember the words.",
                        "\"Open, Barley! Open, Beans!\"",
                        "He called out every grain he knew, but the rock did not move.",
                        "And while he stood there, the thieves came back."
                    ]
                }
            ]
        },
        {
            num: 4,
            title: 'Chapter 4 · The Mark on the Door',
            beats: [
                {
                    art: '04-chalk.webp',
                    emoji: '🚪',
                    left: [
                        "Cassim did not come home. His wife ran to Ali Baba.",
                        "Next day Ali Baba went to the cave. The thieves had caught Cassim, and he was dead. Ali Baba brought him home on a donkey.",
                        "Cassim's wife sat down and wept. The house was in an uproar.",
                        "Then Morgiana, the servant girl, stepped forward."
                    ],
                    right: [
                        "\"If word of this gets about, the thieves will find us,\" said Morgiana. \"Leave it to me.\"",
                        "She told the neighbours her master had died of an illness, and she held the funeral quietly.",
                        "No one guessed about the cave.",
                        "Then Ali Baba's family moved into Cassim's house."
                    ]
                },
                {
                    art: '04-chalk-2.webp',
                    emoji: '🚪',
                    left: [
                        "Meanwhile the thieves saw that Cassim was gone from the cave.",
                        "The captain ground his teeth. \"So somebody else knows about the cave.\"",
                        "One thief went down to the village, going from house to house and asking about a recent funeral.",
                        "At last he found it and put a chalk mark on the door."
                    ],
                    right: [
                        "Next morning Morgiana came out to fetch water. She stopped dead at the sight of the mark.",
                        "'Someone wants to find our house,' she thought.",
                        "Morgiana fetched a piece of chalk at once and drew the same mark on every door in the neighbourhood.",
                        "Twenty houses, thirty houses!"
                    ]
                }
            ]
        },
        {
            num: 5,
            title: 'Chapter 5 · Forty Jars in the Yard',
            beats: [
                {
                    art: '05-jars.webp',
                    emoji: '🏺',
                    left: [
                        "That night the thieves went for nothing. The same mark was on every door.",
                        "\"Which house is it, then?\" roared the captain.",
                        "In the end the captain went himself. This time he made no mark. He fixed the shape of the house in his head instead."
                    ],
                    right: [
                        "A few days later a guest came to Ali Baba's house, dressed as an oil merchant.",
                        "\"Might I have a bed for one night?\" he asked.",
                        "Ali Baba gave him a room readily enough.",
                        "The guest set down his great jars in the yard. There were forty of them."
                    ]
                },
                {
                    art: '05-jars-2.webp',
                    emoji: '🏺',
                    left: [
                        "The night grew late. Morgiana went to light the lamp and found the oil was gone.",
                        "'I shall take a little from the jars in the yard,' she thought.",
                        "She went up to one of them, and a voice whispered from inside. \"Captain — is it now?\""
                    ],
                    right: [
                        "Morgiana's breath stopped. But she made her voice deep and answered as if she were the captain. \"Not yet. Wait.\"",
                        "\"Yes, captain,\" came the answer.",
                        "She went from jar to jar, and from each one came the same question.",
                        "Only one of them held any oil at all."
                    ]
                }
            ]
        },
        {
            num: 6,
            title: "Chapter 6 · Morgiana's Dance",
            beats: [
                {
                    art: '06-dance.webp',
                    emoji: '🔥',
                    left: [
                        "Morgiana did not hurry. She went to the jar with the oil in it, drew off a great potful, and set it boiling in the kitchen.",
                        "Bubble, bubble.",
                        "The steam went up to the ceiling. Then she carried the pot out into the yard."
                    ],
                    right: [
                        "She poured the hot oil into every jar.",
                        "\"Ow! Hot, hot!\"",
                        "The thieves leapt out of the jars, hopping about. They went over the wall and away in every direction.",
                        "The captain heard the noise from his room and slipped out by the back door."
                    ]
                },
                {
                    art: '06-dance-2.webp',
                    emoji: '💃',
                    left: [
                        "Some time later the captain came back, this time dressed as a merchant.",
                        "Little by little he made friends with Ali Baba's son, and he got himself invited to supper.",
                        "But Morgiana, laying the table, saw his face. 'I have seen that face somewhere,' she thought."
                    ],
                    right: [
                        "She thought hard. Then it came to her. 'The oil merchant!'",
                        "After supper Morgiana asked leave to dance, with a small dagger at her waist.",
                        "Boom, boom, boom! She turned and turned to the drum.",
                        "The dance stopped dead in front of the guest. With the tip of her dagger, she brushed his coat open."
                    ]
                }
            ]
        },
        {
            num: 7,
            title: 'Chapter 7 · Found Out',
            beats: [
                {
                    art: '07-revealed.webp',
                    emoji: '⚔️',
                    left: [
                        "In that instant everyone saw a great sword hidden at his waist.",
                        "\"Master, look at this sword!\" Morgiana cried. \"This is the oil merchant from the other night!\"",
                        "The room froze in a moment."
                    ],
                    right: [
                        "The son dropped the cup he was holding. The captain sprang up and made for the door.",
                        "But the doorway was packed with neighbours, standing guard.",
                        "Morgiana had called them in before she began to dance.",
                        "The captain was taken there and then."
                    ]
                },
                {
                    art: '07-revealed-2.webp',
                    emoji: '⚔️',
                    left: [
                        "Ali Baba was silent for a long while. Then he looked at Morgiana.",
                        "\"You have saved this whole house,\" he said. \"You are no servant now.\"",
                        "Morgiana smiled shyly. \"I only did what I had to do.\""
                    ],
                    right: [
                        "Later, Ali Baba went back to the cave with his son. He shared the gold evenly among the villagers.",
                        "With it they dug a well and mended the broken road.",
                        "But he told the magic words to nobody outside his family."
                    ]
                }
            ]
        }
    ],
    quiz: [
        {
            q: 'What did Cassim shout when he tried to get out of the cave?',
            choices: ['Open, Sesame', 'Shut, Sesame', 'Open, Barley'],
            answer: 2
        },
        {
            q: 'What did Ali Baba carry home from the cave the first time?',
            choices: ['Three sacks of gold coins', 'A sack of silk and jewels', 'A single gold coin'],
            answer: 0
        },
        {
            q: 'How did Cassim’s wife find out that Ali Baba had gold?',
            choices: ['Ali Baba’s wife boasted about it', 'She waxed the bottom of the measure she lent them', 'She followed Ali Baba up the hill'],
            answer: 1
        },
        {
            q: 'How did the thief find Cassim’s house?',
            choices: ['He asked around for a house that had just held a funeral', 'He saw the coin stuck to the measure', 'He made friends with Ali Baba’s son and got invited in'],
            answer: 0
        },
        {
            q: 'What did Morgiana do once she knew thieves were hiding in the jars?',
            choices: ['She called the neighbours to guard the jars', 'She poured boiling oil into every jar', 'She locked the door of the captain’s room'],
            answer: 1
        },
        {
            q: 'What did Morgiana do in front of the guest while she was dancing?',
            choices: ['She flicked his coat open with her dagger and showed the hidden sword', 'She dropped a cup as a signal to the neighbours', 'She held the lamp up to his face'],
            answer: 0
        },
        {
            q: 'What did Ali Baba say to Morgiana after the captain was caught?',
            choices: ['I will give you this house', 'You are no longer a servant', 'We will share the gold half and half'],
            answer: 1
        },
        {
            q: 'Which reaction to this book does NOT fit?',
            wide: true,
            choices: [
                "Cassim took ten mules with him — he meant to bring back a share for his brother too.",
                "Ali Baba said not to count it, just to bury it — he was afraid it would show.",
                "Morgiana chalked the same mark on every door in the street — instead of rubbing it out she made it useless.",
                "She called the neighbours in before she began to dance — she never meant to face him alone."
            ],
            answer: 0
        }
    ],
    afterword: {
        title: 'After Reading',
        emoji: '🏺',
        spreads: [
            {
                art: 'end.webp',
                left: [
                    "Ali Baba was not in the Arabian Nights to begin with. A Frenchman added it about three hundred years ago.",
                    "Ali Baba was a poor woodcutter. He only saw the thieves open that door because he had gone up the hill to cut wood.",
                    "His brother forgot the words 'Open, Sesame' and was shut in the cave. Ali Baba had said them over to himself again and again.",
                    "His sister-in-law lent them a measure with wax on the bottom, and a gold coin came back stuck to it. That is how his brother found out."
                ],
                right: [
                    "Morgiana, the servant, saves lives most often in this story.",
                    "How much should Ali Baba have taken out of that cave?"
                ]
            }
        ]
    },
    words: {
        'cover.webp': [
            { word: 'woodcutter', meaning: '나무꾼', sentence: 'Ali Baba, a poor woodcutter.' },
            { word: 'band', meaning: '떼, 무리', sentence: 'A band of robbers in the hills.' },
            { word: 'secret', meaning: '비밀', sentence: 'He learns their secret.' },
            { word: 'household', meaning: '집안 식구', sentence: 'What will happen to his household now that he knows it?' }
        ],
        '01-cave.webp': [
            { word: 'axe', meaning: '도끼', sentence: 'One axe and three donkeys.' },
            { word: 'own', meaning: '가지다', sentence: 'That was everything Ali Baba owned.' },
            { word: 'hooves', meaning: '말발굽', sentence: 'The sound of hooves came shaking the ground.' },
            { word: 'scramble up', meaning: '기어오르다', sentence: 'Ali Baba scrambled quickly up a tree.' }
        ],
        '01-cave-2.webp': [
            { word: 'count on one’s fingers', meaning: '손가락을 꼽아 세다', sentence: 'Ali Baba counted them on his fingers.' },
            { word: 'captain', meaning: '두목', sentence: 'A man who looked like the captain came forward.' },
            { word: 'split apart', meaning: '쩍 갈라지다', sentence: 'And the rock split apart.' },
            { word: 'empty-handed', meaning: '빈손으로', sentence: 'They all came out empty-handed.' },
            { word: 'seam', meaning: '이음매', sentence: 'The rock closed up without a seam.' }
        ],
        '02-treasure.webp': [
            { word: 'hang about', meaning: '망설이며 서성이다', sentence: 'He hung about behind the tree for a long while.' },
            { word: 'screw up one’s courage', meaning: '큰맘 먹다', sentence: 'Then he screwed up his courage.' },
            { word: 'dazzle', meaning: '눈부시게 하다', sentence: 'Inside, the light dazzled him.' },
            { word: 'silk', meaning: '비단', sentence: 'There were silks and jewels besides.' }
        ],
        '02-treasure-2.webp': [
            { word: 'divide', meaning: '나누다', sentence: 'He divided it into three sacks.' },
            { word: 'heap on', meaning: '수북이 덮다', sentence: 'And heaped firewood on top.' },
            { word: 'bury', meaning: '묻다', sentence: 'Never mind counting. Let us bury it.' },
            { word: 'measure', meaning: '됫박', sentence: 'She went to borrow a measure.' }
        ],
        '03-brother.webp': [
            { word: 'odd', meaning: '이상한', sentence: "Cassim's wife thought it very odd." },
            { word: 'smear', meaning: '바르다', sentence: 'She smeared a little wax on the bottom.' },
            { word: 'wax', meaning: '밀랍', sentence: 'A gold coin was stuck fast in the wax.' },
            { word: 'leave nothing out', meaning: '빠짐없이 말하다', sentence: 'He told the whole thing, leaving nothing out.' },
            { word: 'greedy', meaning: '욕심내는', sentence: 'You must not be greedy about this.' }
        ],
        '03-brother-2.webp': [
            { word: 'mule', meaning: '노새', sentence: 'He took ten mules with him.' },
            { word: 'lose one’s head', meaning: '정신이 나가다', sentence: 'Cassim quite lost his head.' },
            { word: 'to the brim', meaning: '그득그득', sentence: 'He filled sack after sack to the brim.' },
            { word: 'barley', meaning: '보리', sentence: 'Open, Barley!' }
        ],
        '04-chalk.webp': [
            { word: 'weep', meaning: '울다', sentence: "Cassim's wife sat down and wept." },
            { word: 'uproar', meaning: '발칵 뒤집힘', sentence: 'The house was in an uproar.' },
            { word: 'get about', meaning: '소문이 나다', sentence: 'If word of this gets about, the thieves will find us.' },
            { word: 'funeral', meaning: '장례', sentence: 'She held the funeral quietly.' }
        ],
        '04-chalk-2.webp': [
            { word: 'ask about', meaning: '수소문하다', sentence: 'Going from house to house and asking about.' },
            { word: 'chalk mark', meaning: '분필 표시', sentence: 'He put a chalk mark on the door.' },
            { word: 'stop dead', meaning: '걸음을 뚝 멈추다', sentence: 'She stopped dead at the sight of the mark.' },
            { word: 'neighbourhood', meaning: '동네', sentence: 'On every door in the neighbourhood.' }
        ],
        '05-jars.webp': [
            { word: 'go for nothing', meaning: '헛걸음하다', sentence: 'That night the thieves went for nothing.' },
            { word: 'fix in one’s head', meaning: '눈에 담아 두다', sentence: 'He fixed the shape of the house in his head.' },
            { word: 'merchant', meaning: '장수, 장사꾼', sentence: 'A guest came dressed as an oil merchant.' },
            { word: 'readily', meaning: '선뜻', sentence: 'Ali Baba gave him a room readily enough.' }
        ],
        '05-jars-2.webp': [
            { word: 'lamp', meaning: '등잔', sentence: 'Morgiana went to light the lamp.' },
            { word: 'as if', meaning: '~인 것처럼', sentence: 'She answered as if she were the captain.' },
            { word: 'hold', meaning: '담고 있다', sentence: 'Only one of them held any oil at all.' }
        ],
        '06-dance.webp': [
            { word: 'draw off', meaning: '퍼내다', sentence: 'She drew off a great potful.' },
            { word: 'boil', meaning: '끓이다', sentence: 'And set it boiling in the kitchen.' },
            { word: 'pour', meaning: '붓다', sentence: 'She poured the hot oil into every jar.' },
            { word: 'in every direction', meaning: '뿔뿔이', sentence: 'They went over the wall and away in every direction.' }
        ],
        '06-dance-2.webp': [
            { word: 'little by little', meaning: '슬슬, 조금씩', sentence: 'He made friends little by little with the son.' },
            { word: 'lay the table', meaning: '상을 차리다', sentence: 'Morgiana, laying the table, saw his face.' },
            { word: 'ask leave', meaning: '청하다', sentence: 'Morgiana asked leave to dance.' },
            { word: 'dagger', meaning: '단검', sentence: 'With a small dagger at her waist.' },
            { word: 'brush open', meaning: '스치듯 젖히다', sentence: 'She brushed his coat open.' }
        ],
        '07-revealed.webp': [
            { word: 'hidden', meaning: '숨겨진', sentence: 'A great sword hidden at his waist.' },
            { word: 'freeze', meaning: '얼어붙다', sentence: 'The room froze in a moment.' },
            { word: 'make for', meaning: '~쪽으로 향하다', sentence: 'The captain sprang up and made for the door.' },
            { word: 'stand guard', meaning: '지키고 서다', sentence: 'The neighbours, standing guard.' },
            { word: 'there and then', meaning: '그길로 바로', sentence: 'The captain was taken there and then.' }
        ],
        '07-revealed-2.webp': [
            { word: 'servant', meaning: '하녀, 하인', sentence: 'You are no servant now.' },
            { word: 'evenly', meaning: '고루', sentence: 'He shared the gold evenly among the villagers.' },
            { word: 'dig a well', meaning: '우물을 파다', sentence: 'With it they dug a well.' },
            { word: 'mend', meaning: '고치다', sentence: 'And mended the broken road.' }
        ],
        'end.webp': [
            { word: 'to begin with', meaning: '원래', sentence: 'Ali Baba was not in the Arabian Nights to begin with.' },
            { word: 'say over', meaning: '되뇌다', sentence: 'He had said them over to himself again and again.' },
            { word: 'sister-in-law', meaning: '형수', sentence: 'His sister-in-law lent them a measure.' },
            { word: 'find out', meaning: '알아내다', sentence: 'That is how his brother found out.' }
        ]
    }
};

/* 글은 두 벌이다. 위쪽 단추를 누르면 EN 쪽으로 갈아 끼우고 쪽을 다시 짠다.
   영어 원고가 없는 책은 단추가 아예 뜨지 않는다. */
/* ── 우리말 낱말 ────────────────────────────────────────────
   쪽마다 아이가 막히는 우리말을 골라 둔다. 영어 낱말과는 고르는 잣대가 다르다.
   영어는 숙어가 걸림돌이고, 우리말은 옛말이나 잘 안 쓰는 말이 걸림돌이다. */
const WORDS_KO = {
    "cover.webp": [
        { w: "실리다", k: "글이나 그림이 책에 들어가다.", s: "아라비안나이트, 곧 천일야화에 실린 이야기예요." },
        { w: "우연히", k: "뜻하지 않게 저절로.", s: "가난한 나무꾼 알리바바가 산에서 우연히 도둑 떼의 비밀을 엿보면서 이야기가 시작돼요." },
        { w: "엿보다", k: "남이 모르게 몰래 보다.", s: "가난한 나무꾼 알리바바가 산에서 우연히 도둑 떼의 비밀을 엿보면서 이야기가 시작돼요." }
    ],
    "01-cave.webp": [
        { w: "살림", k: "한집에서 먹고 지내는 데 쓰는 것들.", s: "알리바바의 살림은 그것이 전부였습니다." },
        { w: "제일가다", k: "여럿 가운데 가장 뛰어나다.", s: "형 카심은 같은 마을에서 제일가는 부자였지요." },
        { w: "나무를 하다", k: "불을 지필 나무를 베어 모으다.", s: "그날도 알리바바는 새벽에 산으로 나무를 하러 갔지요." },
        { w: "숨을 죽이다", k: "소리가 나지 않게 숨을 참다.", s: "숨을 죽이고 내려다보니 말 탄 사람들이 몰려오고 있었지요." }
    ],
    "01-cave-2.webp": [
        { w: "하나같이", k: "여럿이 모두 똑같이.", s: "하나같이 천으로 얼굴을 가리고 있었습니다." },
        { w: "우뚝", k: "꼼짝 않고 높이 솟은 듯이.", s: "사내는 커다란 바위 앞에 우뚝 섰지요." },
        { w: "도적", k: "남의 물건을 훔치는 사람. 도둑.", s: "도적들이 줄줄이 안으로 들어가더니, 한참 뒤에 다들 빈손으로 나왔지요." },
        { w: "감쪽같이", k: "붙인 자리를 알아볼 수 없게.", s: "그러자 바위가 감쪽같이 다시 붙었지요." }
    ],
    "02-treasure.webp": [
        { w: "주문", k: "마법을 부릴 때 외는 말.", s: "아까 들은 주문을 입속으로 몇 번이나 중얼거려 보았지요." },
        { w: "입속으로", k: "남이 듣지 못할 만큼 작게.", s: "아까 들은 주문을 입속으로 몇 번이나 중얼거려 보았지요." },
        { w: "금화", k: "금으로 만든 돈.", s: "금화가 산처럼 쌓여 있었습니다." }
    ],
    "02-treasure-2.webp": [
        { w: "나뭇짐", k: "베어서 묶어 놓은 나무 짐.", s: "그 위에는 나뭇짐을 수북이 덮었습니다." },
        { w: "손을 젓다", k: "손을 흔들어 아니라고 하다.", s: "알리바바가 손을 저었지요." },
        { w: "재다", k: "길이나 양이 얼마인지 알아보다.", s: "하나하나 세기엔 너무 많아, 곡식 양을 재는 나무 그릇인 됫박으로 재어 보기로 했지요." }
    ],
    "03-brother.webp": [
        { w: "닢", k: "동전을 셀 때 쓰는 말.", s: "돌려받은 됫박을 뒤집어 보니 밀랍에 금화 한 닢이 딱 붙어 있었지요." },
        { w: "다그치다", k: "대답하라고 세게 몰아붙이다.", s: "카심은 당장 동생을 찾아가 다그쳤습니다." },
        { w: "말리다", k: "하지 말라고 막다.", s: "알리바바가 말렸지요." },
        { w: "들은 척도 하지 않다", k: "남의 말을 아예 듣지 않다.", s: "그러나 카심은 들은 척도 하지 않았습니다." }
    ],
    "03-brother-2.webp": [
        { w: "싱글벙글", k: "기분이 좋아 소리 없이 웃는 모양.", s: "자루를 다 채운 카심은 싱글벙글했습니다." },
        { w: "막상", k: "정말로 그때가 되니.", s: "그런데 막상 나가려니 주문이 떠오르지 않았지요." },
        { w: "꿈쩍하다", k: "조금 움직이다.", s: "생각나는 곡식 이름을 다 대 보았지만 바위는 꿈쩍도 하지 않았습니다." }
    ],
    "04-chalk.webp": [
        { w: "목숨을 잃다", k: "죽다.", s: "형은 도적들에게 들켜 이미 목숨을 잃은 뒤였지요." },
        { w: "모시다", k: "어른을 받들어 데려오다.", s: "알리바바는 형을 당나귀에 싣고 몰래 집으로 모셔 왔습니다." },
        { w: "눈치가 빠르다", k: "남의 마음이나 형편을 얼른 알아차리다.", s: "모르지아나는 눈치가 아주 빨랐지요." },
        { w: "눈치채다", k: "몰래 하는 일을 알아차리다.", s: "동굴 일은 아무도 눈치채지 못했습니다." }
    ],
    "04-chalk-2.webp": [
        { w: "한편", k: "이야기를 다른 쪽으로 옮길 때 쓰는 말.", s: "한편 도적들은 동굴에서 카심이 사라진 것을 알았습니다." },
        { w: "이를 갈다", k: "분해서 꼭 갚아 주겠다고 마음먹다.", s: "두목이 이를 부드득 갈았지요." },
        { w: "대문", k: "집 앞쪽에 있는 큰 문.", s: "도적은 대문에 분필로 슬쩍 표시를 해 두었지요." },
        { w: "물을 긷다", k: "우물에서 물을 떠 오다.", s: "이튿날 아침 모르지아나가 물을 길러 나왔지요." }
    ],
    "05-jars.webp": [
        { w: "버럭", k: "갑자기 소리를 크게 지르는 모양.", s: "두목이 버럭 소리를 질렀지요." },
        { w: "차림", k: "겉으로 갖추어 입은 모습.", s: "기름 장수 차림이었지요." },
        { w: "묵다", k: "남의 집에서 밤을 지내다.", s: "\"하룻밤만 묵게 해 주십시오.\"" },
        { w: "부려 놓다", k: "실어 온 짐을 내려놓다.", s: "손님은 싣고 온 큰 항아리를 마당에 부려 놓았지요." }
    ],
    "05-jars-2.webp": [
        { w: "덜다", k: "많은 데에서 얼마를 떼어 내다.", s: "\"마당 항아리에서 좀 덜어 오자.\"" },
        { w: "소곤소곤", k: "남이 못 듣게 작게 말하는 소리.", s: "모르지아나가 항아리에 다가가자, 안에서 소곤소곤 소리가 났지요." },
        { w: "숨이 멎다", k: "너무 놀라 숨을 쉬지 못하다.", s: "모르지아나는 숨이 딱 멎었습니다." },
        { w: "굵다", k: "목소리가 낮고 두껍다.", s: "그래도 목소리를 굵게 낮춰 대답했지요." },
        { w: "등잔", k: "기름을 담아 심지에 불을 붙이던 옛날 등.", s: "모르지아나가 등잔에 불을 켜려는데 기름이 떨어져 있었지요." }
    ],
    "06-dance.webp": [
        { w: "솥", k: "밥이나 국을 끓이는 큰 그릇.", s: "기름이 든 항아리로 가서 기름을 크게 한 솥 펐지요." },
        { w: "팔팔", k: "물이 세게 끓는 모양.", s: "그러고는 부엌에서 팔팔 끓였습니다." },
        { w: "김", k: "뜨거운 것에서 올라오는 하얀 물기.", s: "김이 천장까지 올라갔지요." },
        { w: "소란", k: "시끄럽고 어수선한 일.", s: "안에서 소란이 일었습니다." }
    ],
    "06-dance-2.webp": [
        { w: "곰곰이", k: "여러 번 깊이 생각하며.", s: "모르지아나는 곰곰이 생각하다 무릎을 쳤지요." },
        { w: "무릎을 치다", k: "잊었던 것이 갑자기 떠오르다.", s: "모르지아나는 곰곰이 생각하다 무릎을 쳤지요." },
        { w: "차다", k: "몸에 걸거나 매달다.", s: "허리에는 작은 단검을 차고 있었지요." },
        { w: "옷자락", k: "옷의 아래로 늘어진 부분.", s: "모르지아나는 단검 끝으로 손님의 옷자락을 휙 젖혔지요." }
    ],
    "07-revealed.webp": [
        { w: "드러나다", k: "가려져 있던 것이 보이게 되다.", s: "그 순간 손님의 옷 안이 드러났습니다." },
        { w: "순식간에", k: "아주 짧은 사이에.", s: "방 안이 순식간에 얼어붙었지요." },
        { w: "빽빽이", k: "틈이 없을 만큼 촘촘하게.", s: "그런데 문 앞에 사람들이 빽빽이 서 있었지요." }
    ],
    "07-revealed-2.webp": [
        { w: "종", k: "남의 집에서 시키는 일을 하며 사는 사람.", s: "이제 자네는 종이 아닐세." },
        { w: "수줍다", k: "부끄러워 낯을 붉히고 머뭇거리다.", s: "모르지아나는 수줍게 웃었습니다." },
        { w: "다만", k: "그런데 한 가지는.", s: "다만 주문만은 아무에게도 알리지 않았습니다." }
    ],
    "end.webp": [
        { w: "형수", k: "형의 아내.", s: "형이 굴을 알게 된 것은 형수 때문입니다." },
        { w: "됫박", k: "쌀이나 콩이 얼마나 되는지 재는 그릇.", s: "됫박 바닥에 밀랍을 발라 두었더니 금화가 붙어 나왔고, 알리바바는 더 숨길 수 없었지요." },
        { w: "하녀", k: "남의 집에서 집안일을 하는 여자.", s: "이 이야기에서 가장 여러 번 사람을 살린 것은 하녀 모르지아나입니다." }
    ]
};

const UI = {
    ko: {
        toc: '차례', quiz: '이야기 문제', after: '읽고 나서',
        home: '학습 허브로 돌아가기', other: 'EN', otherAria: 'Read in English',
        page: n => `${n}쪽`, wordsDown: '낱말 ⌄',
        done: (n, all) => `${n} / 총 ${all}문항 완료`
    },
    en: {
        toc: 'Contents', quiz: 'Story Questions', after: 'After Reading',
        home: 'Back to the learning hub', other: '한국어', otherAria: '한국어로 읽기',
        page: n => `p. ${n}`, wordsDown: 'Words ⌄',
        done: (n, all) => `${n} of ${all} answered`
    }
};

const LANG_KEY = 'world-tales-lang';
const HAS_EN = typeof EN !== 'undefined';
const readLang = () => { try { return localStorage.getItem(LANG_KEY); } catch (e) { return null; } };
const saveLang = v => { try { localStorage.setItem(LANG_KEY, v); } catch (e) { /* 저장이 막힌 곳도 있다 */ } };

let LANG = (HAS_EN && readLang() === 'en') ? 'en' : 'ko';

const T  = () => UI[LANG];
const CH = () => (LANG === 'en' ? EN.chapters  : CHAPTERS);
const QZ = () => (LANG === 'en' ? EN.quiz      : QUIZ);
const AF = () => (LANG === 'en' ? EN.afterword : AFTERWORD);
const CV = () => (LANG === 'en' ? EN.cover     : COVER);

const TWO_PAGE_KINDS = new Set(['spread', 'toc', 'cover', 'after']);

/* 말을 바꾸면 쪽을 다시 짠다. 쪽 수는 두 말이 같으므로 보던 자리가 흔들리지 않는다. */
let PAGES = [];
let FOLIOS = [];

function buildPages() {
    const chs = CH();
    PAGES = [
    { kind: 'cover' },
    { kind: 'toc' },
    ...chs.flatMap((chapter, ci) => chapter.beats.map((beat, i) => ({
        kind: 'spread', chapter, beat,
        isFirst: i === 0,
        isLast: ci === chs.length - 1 && i === chapter.beats.length - 1
    }))),
    { kind: 'quiz' },
    ...AF().spreads.map((spread, i) => ({ kind: 'after', spread, isFirst: i === 0 }))
    ];

    let folioCounter = 0;
    FOLIOS = PAGES.map(p => {
        const width = TWO_PAGE_KINDS.has(p.kind) ? 2 : 1;
        const start = folioCounter + 1;
        folioCounter += width;
        return { start, width };
    });
}

function renderPage(page) {
    switch (page.kind) {
        case 'cover':
            return coverPage();
        case 'toc':
            return tocPage();
        case 'spread':
            return spreadPage(page.chapter, page.beat, page.isFirst, page.isLast);
        case 'after':
            return afterPage(page.spread, page.isFirst);
        case 'quiz':
            return quizPage();
        default:
            return '';
    }
}

let current = 0;
let animating = false;

const spreadEl = document.getElementById('spread');
const prevBtn = document.getElementById('prevBtn');
const nextBtn = document.getElementById('nextBtn');
const indicatorEl = document.getElementById('pageIndicator');
const folioLeftEl = document.getElementById('folioLeft');
const folioRightEl = document.getElementById('folioRight');

function paint() {
    spreadEl.innerHTML = renderPage(PAGES[current]);
    prevBtn.disabled = current === 0;
    nextBtn.disabled = current === PAGES.length - 1;
    indicatorEl.textContent = `${current + 1} / ${PAGES.length}`;

    const folio = FOLIOS[current];
    folioLeftEl.classList.toggle('folio-center', folio.width === 1);
    if (folio.width === 2) {
        folioLeftEl.textContent = folio.start;
        folioRightEl.textContent = folio.start + 1;
        folioLeftEl.hidden = false;
        folioRightEl.hidden = false;
    } else {
        folioLeftEl.textContent = folio.start;
        folioLeftEl.hidden = false;
        folioRightEl.hidden = true;
    }

    spreadEl.querySelectorAll('[data-goto]').forEach(btn => {
        btn.addEventListener('click', () => {
            const chapterNum = Number(btn.dataset.goto);
            const idx = PAGES.findIndex(p => p.kind === 'spread' && p.chapter.num === chapterNum && p.isFirst);
            if (idx >= 0) goTo(idx);
        });
    });
    spreadEl.querySelectorAll('[data-goto-kind]').forEach(btn => {
        btn.addEventListener('click', () => {
            const idx = PAGES.findIndex(p => p.kind === btn.dataset.gotoKind);
            if (idx >= 0) goTo(idx);
        });
    });

    if (PAGES[current].kind === 'quiz') {
        initQuiz();
    }

    const readBtn = document.getElementById('readBtn');
    if (readBtn) readBtn.addEventListener('click', () => (reading ? stopReading() : readPage(0)));

    // 읽는 중일 때만 문단을 눌러 그 자리로 옮긴다.
    if (LANG === 'en' && CAN_SPEAK) {
        spreadEl.querySelectorAll('[data-say]').forEach(el => {
            el.addEventListener('click', () => {
                if (!reading) return;
                readPage(Number(el.dataset.say));
            });
        });
    }
    if (typeof renderVocab === 'function') renderVocab();
}

function initQuiz() {
    const progressEl = document.getElementById('quizProgress');

    spreadEl.querySelectorAll('.quiz-item').forEach(item => {
        const qi = Number(item.dataset.qindex);
        const q = QZ()[qi];
        item.querySelectorAll('.quiz-choice').forEach(btn => {
            btn.addEventListener('click', () => {
                if (item.classList.contains('graded')) return;
                const ci = Number(btn.dataset.choice);
                // 맞힐 때까지 다시 고를 수 있다. 틀린 것만 빨갛게 남는다.
                // 답을 미리 보여 주면 아이가 생각할 자리가 사라진다.
                if (ci !== q.answer) {
                    btn.classList.add('incorrect');
                    (QUIZ_WRONG[QK(qi)] = QUIZ_WRONG[QK(qi)] || []).push(ci);
                    return;
                }
                btn.classList.add('correct');
                item.classList.add('graded');
                QUIZ_DONE[QK(qi)] = true;
                progressEl.textContent = T().done(quizDone(), QZ().length);
            });
        });
    });
}

function goTo(index) {
    if (animating || index === current || index < 0 || index >= PAGES.length) return;
    stopReading();
    animating = true;
    const dir = index > current ? 'flip-next' : 'flip-prev';
    spreadEl.classList.add(dir);
    setTimeout(() => {
        current = index;
        paint();
    }, 230);
    setTimeout(() => {
        spreadEl.classList.remove('flip-next', 'flip-prev');
        animating = false;
    }, 480);
}

prevBtn.addEventListener('click', () => goTo(current - 1));
nextBtn.addEventListener('click', () => goTo(current + 1));

document.getElementById('tocLink').addEventListener('click', () => {
    const idx = PAGES.findIndex(p => p.kind === 'toc');
    if (idx >= 0) goTo(idx);
});

document.addEventListener('keydown', (e) => {
    if (e.key === 'ArrowRight') goTo(current + 1);
    if (e.key === 'ArrowLeft') goTo(current - 1);
});

/* ── 목소리 ────────────────────────────────────────────────────
   기기에 있는 영어 목소리를 골라 쓴다. 없으면 기본 목소리로 읽는다. */
const textOf = p => (typeof p === 'string' ? p : p.t);

/* 읽기 단추 — 표지·펼침면·읽고 나서에 붙는다. 영어로 볼 때만 나온다. */
function readBtnHtml() {
    return (LANG === 'en' && CAN_SPEAK)
        ? `<button type="button" class="read-btn" id="readBtn">${reading ? '■' : '▶'}</button>`
        : '';
}

/* 그 쪽에서 읽어 줄 글. 화면에 보이는 차례 그대로다. */
function pageParts(page) {
    if (!page) return [];
    if (page.kind === 'spread') return page.beat.left.concat(page.beat.right);
    if (page.kind === 'cover') return [CV().title].concat(CV().intro);
    if (page.kind === 'after') return page.spread.left.concat(page.spread.right);
    return [];
}

const SAY_RATE = 0.85;
const VOICES = { any: null };

function pickVoices() {
    if (typeof speechSynthesis === 'undefined') return;
    const vs = speechSynthesis.getVoices().filter(v => /^en/i.test(v.lang));
    if (!vs.length) return;
    VOICES.any = vs.find(v => /^en[-_]US/i.test(v.lang)) || vs[0];
}

if (typeof speechSynthesis !== 'undefined') {
    pickVoices();
    speechSynthesis.onvoiceschanged = pickVoices;
}

function dressVoice(u) {
    u.rate = SAY_RATE;
    if (VOICES.any) u.voice = VOICES.any;
}

/* ── 읽어 주기 ─────────────────────────────────────────────────
   펼침면 글을 왼쪽부터 차례로 읽는다. 읽는 문단에 표시가 따라간다.
   다시 누르면 멈춘다. 쪽을 넘기거나 우리말로 돌아가면 저절로 멈춘다. */
let reading = false;
let readToken = 0;

function stopReading() {
    reading = false;
    if (typeof spreadEl !== 'undefined' && spreadEl) spreadEl.classList.remove('is-reading');
    readToken++;
    if (CAN_SPEAK) { try { speechSynthesis.cancel(); } catch (e) {} }
    document.querySelectorAll('.saying').forEach(el => el.classList.remove('saying'));
    const b = document.getElementById('readBtn');
    if (b) b.textContent = '▶';
}

function readPage(from) {
    const page = PAGES[current];
    if (!CAN_SPEAK || !page) return;
    const parts = pageParts(page);
    if (!parts.length) return;
    try { speechSynthesis.cancel(); } catch (e) {}
    reading = true;
    if (spreadEl) spreadEl.classList.add('is-reading');
    const mine = ++readToken;
    const btn = document.getElementById('readBtn');
    if (btn) btn.textContent = '■';

    const step = (i) => {
        if (mine !== readToken) return;
        document.querySelectorAll('.saying').forEach(el => el.classList.remove('saying'));
        if (i >= parts.length) { stopReading(); return; }
        const here = document.querySelector(`[data-say="${i}"]`);
        if (here) {
            here.classList.add('saying');
            here.scrollIntoView({ block: 'nearest' });
        }
        const u = new SpeechSynthesisUtterance(textOf(parts[i]).replace(/"/g, ''));
        u.lang = 'en-US';
        dressVoice(u);
        u.onend = () => step(i + 1);
        u.onerror = () => step(i + 1);
        try { speechSynthesis.speak(u); } catch (e) { stopReading(); }
    };
    step(Math.max(0, Math.min(from | 0, parts.length - 1)));
}

/* ── 단어장 — 영어로 읽을 때만 책 아래에 깔린다 ──────────────────── */
const vocabScreenEl = document.getElementById('vocabScreen');
const vocabPanelEl = document.getElementById('vocabPanel');
const scrollDownEl = document.getElementById('scrollDown');
const HAS_WORDS = HAS_EN && EN.words && Object.keys(EN.words).length > 0;
const HAS_WORDS_KO = typeof WORDS_KO !== 'undefined' && WORDS_KO && Object.keys(WORDS_KO).length > 0;

/* 듣기 — 낱말을 먼저 읽고, 이어서 그 낱말이 나온 구절을 읽는다.
   목소리가 없는 기기에서는 단추 자체가 뜨지 않는다. */
const CAN_SPEAK = typeof speechSynthesis !== 'undefined';
let VOCAB_NOW = [];

function sayWord(item) {
    if (!CAN_SPEAK || !item) return;
    try {
        speechSynthesis.cancel();
        const bare = String(wOf(item)).replace(/\s*\([^)]*\)/g, '').trim();
        const 말 = LANG === 'en' ? 'en-US' : 'ko-KR';
        const word = new SpeechSynthesisUtterance(bare);
        word.lang = 말;
        word.rate = 0.75;
        const sent = new SpeechSynthesisUtterance(sOf(item));
        sent.lang = 말;
        speechSynthesis.speak(word);
        speechSynthesis.speak(sent);
    } catch (e) { /* 소리를 못 내는 기기도 있다 */ }
}

function vocabFor() {
    const all = LANG === 'en' ? ((HAS_WORDS && EN.words) || {}) : ((HAS_WORDS_KO && WORDS_KO) || {});
    const page = PAGES[current];
    // 그 쪽에 실제로 있는 글의 낱말을, 글에 나온 차례대로 보여 준다.
    const key = !page ? null
        : page.kind === 'spread' ? page.beat.art
        : page.kind === 'after' ? page.spread.art
        : null;
    if (key && all[key]) return all[key];
    // 표지에도 소개글이 있다. 그 글에 나온 낱말만 보여 준다.
    if (page && page.kind === 'cover') return all['cover.webp'] || all['cover'] || [];
    // 차례·문제 쪽에는 글이 없다. 보여 줄 것이 없으면 칸을 접는다.
    return [];
}

/* 낱말 자료는 우리말과 영어가 적는 이름이 다르다. 우리말 쪽은 전래동화와 같은
   {w, k, s}, 영어 쪽은 {word, meaning, sentence}다. 어느 쪽이 와도 읽게 한다. */
const wOf = w => (w.word !== undefined ? w.word : w.w);
const mOf = w => (w.meaning !== undefined ? w.meaning : w.k);
const sOf = w => (w.sentence !== undefined ? w.sentence : w.s);

function renderVocab() {
    const has = LANG === 'en' ? HAS_WORDS : HAS_WORDS_KO;
    const list = has ? vocabFor() : [];
    const on = list.length > 0;
    if (vocabScreenEl) vocabScreenEl.hidden = !on;
    if (scrollDownEl) {
        scrollDownEl.hidden = !on;
        scrollDownEl.textContent = T().wordsDown || (LANG === 'en' ? 'Words ⌄' : '낱말 ⌄');
    }
    if (!on) {
        if (window.scrollY) window.scrollTo({ top: 0 });
        return;
    }
    VOCAB_NOW = list;
    vocabPanelEl.innerHTML = `
        <ul class="vocab-list">
            ${list.map((w, i) => `
            <li>
                <div class="vocab-top">
                    <p class="vocab-word">${wOf(w)}</p>
                    ${(CAN_SPEAK && LANG === 'en') ? `<button type="button" class="vocab-say" data-i="${i}" aria-label="Listen">🔊</button>` : ''}
                </div>
                <p class="vocab-mean">${mOf(w)}</p>
                <p class="vocab-sent">${sOf(w)}</p>
            </li>`).join('')}
        </ul>`;
    fitVocabScreen();
}

if (vocabPanelEl) {
    vocabPanelEl.addEventListener('click', e => {
        const btn = e.target.closest('.vocab-say');
        if (btn) sayWord(VOCAB_NOW[Number(btn.dataset.i)]);
    });
}

/* 그림선 — 그림 칸이 끝나는 자리다. 여기까지만 내려가면 그림만 위로 빠지고
   읽던 글은 화면에 남는다. 아래 화면 높이를 딱 그만큼 주면 더 내려갈 데가
   없어져 저절로 그 자리에 걸린다. */
function artLine() {
    const page = PAGES[current];
    const el = !page ? null
        : page.kind === 'spread' ? document.querySelector('.spread-art')
        : page.kind === 'cover' ? document.querySelector('.page-cover .story-page-left-full')
        : page.kind === 'after' ? document.querySelector('.after-art')
        : null;
    if (!el) return 0;
    const box = el.getBoundingClientRect();
    const line = box.bottom + window.scrollY;
    const book = document.querySelector('.book');
    if (!book) return Math.max(0, Math.round(line));
    const bookBox = book.getBoundingClientRect();
    // 그림이 책 높이를 거의 다 차지하면 경계선이 곧 책 밑이라 책이 통째로
    // 사라진다. 그때만 책이 절반쯤 남도록 붙든다.
    if (box.height < bookBox.height * 0.8) return Math.max(0, Math.round(line));
    const cap = bookBox.bottom + window.scrollY - Math.round(window.innerHeight * 0.45);
    return Math.max(0, Math.round(Math.min(line, Math.max(cap, 0))));
}

function fitVocabScreen() {
    if (!vocabScreenEl || vocabScreenEl.hidden) return;
    const line = artLine();
    // 펼침면이 아닌 쪽(차례·문제)에는 그림 칸이 없다. 그때는 내용만큼만 둔다.
    vocabScreenEl.style.minHeight = line ? line + 'px' : '';
}

if (scrollDownEl) {
    scrollDownEl.addEventListener('click', () => {
        fitVocabScreen();
        const line = artLine();
        window.scrollTo({ top: line || document.documentElement.scrollHeight, behavior: 'smooth' });
    });
}

window.addEventListener('resize', () => { window.scrollTo(0, 0); fitVocabScreen(); });

/* 말 바꾸기 — 보던 자리를 그대로 두고 글만 갈아 끼운다. */
const langBtn = document.getElementById('langLink');
function applyLang() {
    if (typeof stopReading === 'function') stopReading();
    document.documentElement.lang = LANG;
    document.title = CV().title;
    if (langBtn) {
        langBtn.hidden = !HAS_EN;
        langBtn.textContent = T().other;
        langBtn.setAttribute('aria-label', T().otherAria);
    }
}
if (langBtn && HAS_EN) {
    langBtn.addEventListener('click', () => {
        LANG = LANG === 'en' ? 'ko' : 'en';
        saveLang(LANG);
        const here = PAGES[current];
        buildPages();
        current = Math.min(current, PAGES.length - 1);
        // 보던 그림 그대로 선다. 쪽 수가 같으므로 그림 파일 이름으로 찾는다.
        if (here && here.kind === 'spread') {
            const i = PAGES.findIndex(p => p.kind === 'spread' && p.beat.art === here.beat.art);
            if (i >= 0) current = i;
        }
        applyLang();
        paint();
    });
}

buildPages();
applyLang();
paint();

