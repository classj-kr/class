...
function createQuestions(item, pool) {
  ...
  // Descriptions without two geographical keywords use exact sentence completion.
  ...
  if(questions.length!==2)throw Error(item.name+'의 설명이 짧아 두 문제를 만들 수 없습니다.');
  return questions.map((q,i)=>({...q,id:'q'+i,choices:shuffle(q.choices)}));
}
function normalizeProgress(raw) {
  const places={};
  for(const [key,value] of Object.entries(raw?.places||{}).slice(0,5)){
    if(!['reading','quiz','completed'].includes(value?.phase))continue;
    places[key]={phase:value.phase,streak:value.phase==='completed'?2:Math.min(1,Math.max(0,Math.floor(Number(value.streak)||0))),
...
function publicSession(target,state) {
  ...
  return {key:target.key,name:target.name,type:target.type,phase:state.phase,streak:state.streak,
...
function answer(target,state,token,choice) {
  ...
  if(correct){state.streak++;if(state.streak===2){state.phase='completed';state.completedAt=Date.now();}else issue(state);}
  else{state.streak=0;state.phase='reading';}
  return {correct,explanation:q.explanation};
}
module.exports={createQuestions,normalizeProgress,publicState,publicSession,issue,answer};
