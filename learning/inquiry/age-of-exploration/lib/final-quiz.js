...
const REGIONAL_QUESTIONS = Object.freeze({
  '이베리아': {
    ...
    choices: ['바닷길과 내륙 유통망을 연결하는 교역 거점', '해상 운송에 필요한 선박을 전문적으로 생산하는 공업 거점', '대서양과 지중해의 함대를 서로 연결하는 군사 거점', '농산물의 생산 시기를 조절하여 가격을 안정시키는 관리 거점'],
    answerIndex: 0,
  ...
  '프랑스': {
    ...
    choices: ['강과 해안을 잇는 여러 경로가 상품 이동을 뒷받침했을 것이다', '한 도시의 시장 가격이 전국의 거래 조건을 결정했을 것이다', '두 바다를 잇는 뱃길이 강을 이용한 운송을 대신했을 것이다', '항구 주변의 농업 생산이 내륙 시장의 형성을 이끌었을 것이다'],
    answerIndex: 0,
  ...
  '이탈리아': {
    ...
    choices: ['서로 다른 교역 경로와 상업 정보가 도시에서 결합했기 때문이다', '도시 사이의 경쟁이 상품별 거래 조건을 조정했기 때문이다', '산악 지형이 도시 사이의 경쟁을 줄여 주었기 때문이다', '알프스를 넘는 육로가 바닷길을 대신해 도시들을 이어 주었기 때문이다'],
    answerIndex: 0,
  ...
  '브리튼': {
    ...
    choices: ['내륙 운송 경로와 바닷물 높이의 변화를 함께 살폈을 것이다', '대서양 쪽과 북해 쪽 항구의 날씨 차이를 함께 비교했을 것이다', '강 하구 시장보다 섬 중앙의 산길을 거래의 중심으로 삼았을 것이다', '항구에 도착한 상품을 해상 운송에 다시 싣는 일을 줄였을 것이다'],
    answerIndex: 0,
  ...
  '북유럽': {
    ...
    choices: ['날씨가 좋은 계절에 맞추어 내륙 물품이 모이는 때를 따졌을 것이다', '해협을 지나는 순서와 선원의 교대 시기를 함께 고려했을 것이다', '연안 항로보다 대양 횡단을 이용해 이동 거리를 줄였을 것이다', '상품의 보관 기간을 줄이기 위해 겨울 출항을 우선했을 것이다'],
    answerIndex: 0,
  ...
  '흑해': {
    ...
    choices: ['강 유역에서 온 물품이 항구를 거쳐 해협 너머 시장으로 나갔다', '해협 주변의 정치 상황이 내륙 생산량을 직접 결정했다', '지중해 상인이 강 상류까지 같은 선박으로 이동했다', '강 상류의 도시들이 해협을 거치지 않고 지중해와 연결되었다'],
    answerIndex: 0,
  ...
  '근동': {
    ...
    choices: ['서로 다른 운송망에서 온 상품과 정보를 이어 주는 기능', '상품의 생산 지역을 한곳으로 모아 관리하는 기능', '대상 이동을 대신해 해상 운송을 확대하는 기능', '대상로와 연안 항로를 이어 군대를 옮기는 기능'],
    answerIndex: 0,
  ...
  '북아프리카': {
    ...
    choices: ['사막 교역과 해상 교역의 일정이 도시 시장에서 이어졌기 때문이다', '해안의 농업 생산이 사막 지역의 보급 문제를 해결했기 때문이다', '선박이 대상보다 많은 상품을 생산지에서 직접 모았기 때문이다', '사하라 대상로가 지중해 항로와 이어져 소금 생산이 늘었기 때문이다'],
    answerIndex: 0,
  ...
  '인도': {
    ...
    choices: ['출항 시기에 맞추어 항구에 머무는 기간까지 계산하게 했다', '상품의 생산량을 바람의 방향에 맞추어 결정하게 했다', '귀항 항로를 출항 항로보다 짧게 선택하게 했다', '면직물과 향신료의 생산 시기를 함께 앞당기게 했다'],
    answerIndex: 0,
  ...
  '중앙아시아': {
    ...
    choices: ['장거리 이동을 여러 구간으로 이어 주는 보급과 교환의 거점', '대상의 출발지를 정해 교역 방향을 통제하는 행정 거점', '초원의 생산물을 사막 환경에 맞게 가공하는 공업 거점', '초원과 사막의 유목민을 하나의 나라로 이어 주는 정치 거점'],
    answerIndex: 0,
  ...
  '동북아시아': {
    ...
    choices: ['내륙 수로와 연안 항로가 서로 이어져 상품의 이동 범위를 넓혔다', '국가의 통제가 내륙 수로보다 연안 항로의 이용을 늘렸다', '운하가 바다와 이어지지 않아 곡물은 내륙에서 소비되었다', '연안 항로가 발달하면서 강과 운하의 운송 기능이 약해졌다'],
    answerIndex: 0,
  ...
  '서아프리카': {
    ...
    choices: ['사막·강·바다의 교역로가 만나 상품과 정보가 모였기 때문이다', '강을 이용한 운송이 사막 대상의 이동을 대신했기 때문이다', '대서양 항로가 내륙 시장의 상품 가격을 정해 주었기 때문이다', '금 산지와 소금 산지가 한 도시 안에서 함께 개발되었기 때문이다'],
    answerIndex: 0,
  ...
  '동아프리카': {
    ...
    choices: ['내륙 물품이 도착하는 때와 계절풍이 바뀌는 때를 맞추었을 것이다', '상아와 도자기를 같은 배에 함께 실어 내륙으로 보냈을 것이다', '선박의 출항을 늘리기 위해 내륙 운송 시기를 늦추었을 것이다', '항구의 상품 종류를 줄여 바람 변화에 대응했을 것이다'],
    answerIndex: 0,
  ...
  '중앙아메리카': {
    ...
    choices: ['기존의 지역 교류망에 바다를 통한 새로운 연결이 겹쳐졌다', '해안 항구의 성장으로 산지 시장의 거래 시기가 조정되었다', '유럽 선박이 도착하면서 산지와 해안 사이의 연결이 끊어졌다', '새로운 교역 경로가 기존의 상품과 이동 경험을 활용하지 않았다'],
    answerIndex: 0,
  ...
  '서인도제도': {
    ...
    choices: ['서로 규모가 다른 항해를 이어 주는 보급과 항로 판단의 거점', '섬 사이의 상품 생산을 조정하여 대양 교역량을 정하는 거점', '카리브해의 섬들을 하나의 시장으로 이어 생산을 늘리는 거점', '바람과 해류의 영향을 줄이기 위해 선박을 장기간 보관하는 거점'],
    answerIndex: 0,
  ...
  '남동아메리카': {
    ...
    choices: ['넓은 내륙의 물품을 모아 배에 실어 보내기에 유리했기 때문이다', '강 하구가 다른 강들과 이어져 육로 운송이 필요 없었기 때문이다', '항구 선박이 내륙의 육상 운송을 직접 관리했기 때문이다', '대서양 시장의 수요가 강 유역 도시의 출항 시기를 정했기 때문이다'],
    answerIndex: 0,
  ...
  '남서아메리카': {
    ...
    choices: ['고도가 다른 지역의 운송 방식을 이어 주어야 했기 때문이다', '산지의 생산물을 해안 기후에 맞게 재배해야 했기 때문이다', '산지 길과 해안 항구를 같은 운송 수단으로 이어야 했기 때문이다', '계곡의 도시가 항구의 선박 수를 조절해야 했기 때문이다'],
    answerIndex: 0,
...
const GENERAL_QUESTIONS = Object.freeze([
  {
    ...
    choices: ['지도상의 거리와 실제 이동 조건을 서로 다르게 고려했기 때문이다', '상인이 선박의 적재량을 항해사보다 적게 계산했기 때문이다', '항해사가 섬의 시장 가격을 항로 선택에 반영했기 때문이다', '항해사가 상인보다 더 짧은 직선 항로를 찾아냈기 때문이다'],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ['환경 조건에 맞추어 운송 수단을 구간별로 바꾸었다', '화물 일부를 버려 배를 가볍게 만들었다', '하류의 항구를 이용해 목적지를 변경했다', '큰 배의 속도를 낮춰 얕은 구간을 통과했다'],
    answerIndex: 0,
  ...
  },
  {
    ...
    choices: ['항로 거리와 실제 항해 효율 사이의 관계', '식량의 가격과 돛의 제작 비용 사이의 관계', '두 항구 사이의 거리와 선원 수 사이의 관계', '바람의 방향과 도착 항구의 상품 가격 사이의 관계'],
    answerIndex: 0,
...
function uniqueChoices(values, correct) {
  ...
  for (const value of values) {
    ...
    if (!Number.isFinite(n) || n <= 0 || result.includes(n)) continue;
    result.push(n);
  }
  let step = 10;
  while (result.length < 4) {
    const candidate = step % 20 === 0 && correct - step > 0 ? correct - step : correct + step;
...
// 오답을 정답의 위아래로 고르게 흩는다. 오답이 모두 정답보다 크거나 작으면 가장 작은(큰) 수를 골라 맞힐 수 있다.
function spreadChoices(correct, offsets) {
  const pool = [...new Set(offsets.filter((offset) => offset > 0))];
  for (let i = pool.length - 1; i > 0; i -= 1) {
    const j = randomInt(0, i);
    [pool[i], pool[j]] = [pool[j], pool[i]];
  }
  const down = pool.filter((offset) => correct - offset > 0).slice(0, randomInt(0, 3));
  const up = pool.filter((offset) => !down.includes(offset)).slice(0, 3 - down.length);
  return [correct, ...down.map((offset) => correct - offset), ...up.map((offset) => correct + offset)];
}
...
function calculationQuestion() {
  ...
  if (kind === 0) {
    ...
    const choices = uniqueChoices(spreadChoices(answer, [transportCost, margin, 2 * margin, buyPrice]), answer).map((n) => `${n}닢`);
  ...
  if (kind === 1) {
    ...
    const choices = uniqueChoices(spreadChoices(answer, [reserve, sailors * dailyWater, days * dailyWater, 2 * sailors * dailyWater]), answer).map((n) => `${n}통`);
    return shuffledQuestion({
  ...
  const answer = randomInt(4, 8) * 10;
  const capacity = food + water + tool + answer;
  const choices = uniqueChoices(spreadChoices(answer, [food, water, tool, water + tool, food + tool]), answer).map((n) => `${n}상자`);
  return shuffledQuestion({
...
function destinationRegionalQuestion(targetPlace) {
  ...
  if (category === '해협') {
    return {
      ...
      choices: ['통로의 자연조건과 선박이 몰리는 시간대', '해협까지의 직선거리와 싣고 있는 상품의 값', '배의 적재량과 목적지 시장의 상품 가격', '해협 양쪽 항구의 크기와 머물 수 있는 날수'],
      answerIndex: 0,
  ...
  if (['반도', '제도', '열도', '곶'].includes(category)) {
    return {
      ...
      choices: ['연안의 자연조건과 육상 연결을 함께 고려해야 한다', '바람과 해류와 상관없이 가까운 해안에 배를 대면 된다', '짧은 연안 항해는 장거리 항로와 이어지지 않고 따로 운영된다', '바다 쪽 조건이 좋으면 육상 연결은 살피지 않아도 된다'],
      answerIndex: 0,
  ...
  if (['강 하구', '삼각주', '폭포'].includes(category)) {
    return {
      ...
      choices: ['물길의 조건이 달라지는 구간에서 운송을 이어 주는 역할', '강 상류의 생산량을 정해 항구의 거래량을 조절하는 역할', '큰 바닷배가 강 상류까지 곧바로 이어 가도록 돕는 역할', '내륙 물품을 모아 두었다가 강 상류로 되돌려 보내는 역할'],
      answerIndex: 0,
  ...
  if (['산맥', '사막', '지구대', '지협'].includes(category)) {
    return {
      ...
      choices: ['거리와 보급·지형 조건을 함께 비교하는 것', '가장 짧은 길을 골라 이동 시간을 줄이는 것', '물과 식량을 넉넉히 실어 보급 거점을 거치지 않는 것', '거리와 통행료를 함께 따져 가장 싼 길을 고르는 것'],
      answerIndex: 0,
  ...
  return {
    ...
    choices: ['지도상의 위치와 실제 이동 조건을 함께 살피는 것', '지도에 표시된 거리가 가장 짧은 길을 고르는 것', '계절과 상관없이 같은 항로를 정해 두고 쓰는 것', '보급 거점과 날씨를 함께 보되 지도는 참고하지 않는 것'],
    answerIndex: 0,
