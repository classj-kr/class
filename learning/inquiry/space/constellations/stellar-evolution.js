(function(root){
    ...
    function stages(mass,remnant){
        ...
        const early=shared.map(stage=>({...stage}));
        if(large)early[2].description+=' 질량이 큰 별은 연료를 훨씬 빠르게 소모하므로 태양급 별보다 수명이 짧습니다.';
        return [...early,
    ...
    function sample(progress,mass='sun',remnant='neutron'){
        ...
        if(index===4){
            ...
            if(large){radius=mix(giantR,finalR,clamp((u-.06)/.16));burst=Math.exp(-Math.pow((u-.25)/.085,2));}
