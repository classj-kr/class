(function () {
  ...
  function create(api) {
    ...
    function show(topic) {
      ...
      select(remembered[topic] || list.find(lesson=>lesson.id===requested)?.id || list[0].id);
    }
    function select(id) {
    ...
    function renderDiagram() {
      const target=host.querySelector('#lessonDiagram');
      if(!api.renderVisual?.(target,current,{quiz:masked})) window.KoreaVisuals.render(target,current,{quiz:masked});
      if(current.id==='foehn') {
        host.querySelector('.model-location-note').textContent='위 단면의 A·B와 지도의 A·B는 같은 위치입니다. 그림 문제는 별도의 원리 모형으로 출제합니다.';
        host.querySelector('.lesson-locations').hidden=true;
        host.querySelector('.reading-steps li:last-child').textContent='응결·강수 조건을 켜고 끄며 하강 쪽을 비교한다.';
      }
    }
