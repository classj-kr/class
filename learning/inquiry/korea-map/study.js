(function () {
  ...
  function create(api) {
    ...
    function render() {
      ...
      host.innerHTML=`<div class="lesson-navigation"><label class="visually-hidden" for="lessonSelect">학습 개념</label><div><button id="previousLesson" aria-label="이전 개념" ${index===0?'disabled':''}>←</button><select id="lessonSelect">${list.map((l,i)=>`<option value="${l.id}" ${l===current?'selected':''}>${String(i+1).padStart(2,'0')} · ${esc(l.title)}</option>`).join('')}</select><button id="nextLesson" aria-label="다음 개념" ${index===list.length-1?'disabled':''}>→</button></div></div>
      <article class="visual-lesson"><div class="visual-heading"><div><h2>${esc(current.title)}</h2></div><div class="visual-actions"><button class="quick-practice" id="quickPractice">그림 문제</button><button class="mask-button" id="maskConcept" aria-pressed="false">이름 가리기</button></div></div><div id="lessonDiagram"></div>
