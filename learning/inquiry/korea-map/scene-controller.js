(function () {
  ...
  function create(map) {
    ...
    function syncSuspension() {
      flow.setSuspended(!!document.querySelector("dialog[open]") || map.getContainer().classList.contains("is-picking"));
    }
    ...
    function setContext(theme,lesson) {
      if (theme === "terrain" && (state.theme !== theme || state.lesson?.id !== lesson?.id)) state.river = "all";
